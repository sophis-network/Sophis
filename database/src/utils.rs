...
pub fn get_sophis_tempdir() -> TempDir {
    let global_tempdir = std::env::temp_dir();
    let sophis_tempdir = global_tempdir.join("rusty-sophis");
    std::fs::create_dir_all(sophis_tempdir.as_path()).unwrap();
    tempfile::tempdir_in(sophis_tempdir.as_path()).unwrap()
}

/// Creates a DB within a temp directory under `<OS SPECIFIC TEMP DIR>/rusty-sophis`
/// Callers must keep the `TempDbLifetime` guard for as long as they wish the DB to exist.
#[macro_export]
macro_rules! create_temp_db {
    ($conn_builder: expr) => {{
        let db_tempdir = $crate::utils::get_sophis_tempdir();
