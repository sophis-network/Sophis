...
pub enum DatabaseStorePrefixes {
    ...
    // ---- Anti long-range attack ----
    /// Persisted floor of the highest cumulative `blue_work` ever committed
    /// by virtual state on this node. Used to refuse promoting a peer's
    /// chain candidate whose tip carries less work than what we have already
    /// validated locally. See `consensus/src/model/stores/max_chain_work_seen.rs`.
    MaxChainWorkSeen = 62,
