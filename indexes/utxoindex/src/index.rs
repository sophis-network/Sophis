...
use sophis_consensus_core::{BlockHashSet, tx::ScriptPublicKeys, utxo::utxo_diff::UtxoDiff};
use sophis_consensusmanager::{ConsensusManager, ConsensusResetHandler};
use sophis_core::{info, trace};
use sophis_database::prelude::{DB, StoreError, StoreResult};
...
const RESYNC_CHUNK_SIZE: usize = 2048; // Increased from 1k (used in go-sophisd), for quicker resets, while still having a low memory footprint.

/// UtxoIndex indexes `CompactUtxoEntryCollections` by [`ScriptPublicKey`](sophis_consensus_core::tx::ScriptPublicKey),
...
impl UtxoIndexApi for UtxoIndex {
    ...
    fn get_all_outpoints(&self) -> StoreResult<std::collections::HashSet<sophis_consensus_core::tx::TransactionOutpoint>> {
