...
/// `cargo test --release --package sophis-testing-integration --lib --features devnet-prealloc -- rpc_perf_benchmarks::bench_rpc_high_load --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
async fn bench_rpc_high_load() {
    ...
    // Setup for pre-allocated UTXOs and transaction generation (Dilithium address)
    ...
    let txs = generate_tx_dag(utxoset.clone(), spk, TX_COUNT / TX_LEVEL_WIDTH, TX_LEVEL_WIDTH);
    ...
    for _ in 0..num_clients {
        ...
        let handle = tokio::spawn(async move {
            ...
            for _ in 0..num_requests_per_client {
                ...
                client.get_virtual_chain_from_block_v2(*hash, Some(sophis_rpc_core::RpcDataVerbosityLevel::High), None).await.unwrap();
