...
use sophis_core::{info, task::tick::TickService, trace};
use sophis_math::Uint256;
use sophis_notify::scope::VirtualDaaScoreChangedScope;
...
fn create_client_addresses(index: usize, network_id: &NetworkId) -> Vec<Address> {
    ...
    (min_address..max_address)
        .map(|x| {
            Address::new((*network_id).into(), sophis_addresses::Version::PubKeyDilithium, &Uint256::from_u64(x as u64).to_le_bytes())
        })
        .collect_vec()
}

/// `cargo test --package sophis-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::utxos_changed_subscriptions_sanity_check --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
async fn utxos_changed_subscriptions_sanity_check() {
    init_allocator_with_default_settings();
    sophis_core::log::try_init_logger(
        "INFO, sophis_core::time=debug, sophis_rpc_core=debug, sophis_grpc_client=debug, sophis_notify=info, sophis_notify::address::tracker=debug, sophis_notify::listener=debug, sophis_notify::subscription::single=debug, sophis_mining::monitor=debug, sophis_testing_integration::subscribe_benchmarks=trace",
    );
    ...
    // Fold-up
    //
    sophis_core::info!("Signal the daemon to shutdown");
    client.shutdown().await.unwrap();
    sophis_core::warn!("Disconnect the main client");
    client.disconnect().await.unwrap();
    drop(client);

    sophis_core::warn!("Waiting for the daemon to exit...");
    daemon_process.wait().await.expect("failed to wait for the daemon process");
}

/// `cargo test --package sophis-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_daemon --exact --nocapture --ignored -- --rpc=46610 --p2p=46611 --private-key=a2760251adb5b6e8d4514d23397f1631893e168c33f92ff8a7a24f397d355d62 --max-tracked-addresses=1000000 --utxoindex`
///
...
async fn bench_utxos_changed_subscriptions_daemon() {
    init_allocator_with_default_settings();
    sophis_core::log::try_init_logger(
        "INFO, sophis_core::core=trace, sophis_core::time=debug, sophis_rpc_core=debug, sophis_grpc_client=debug, sophis_notify=info, sophis_notify::address::tracker=debug, sophis_notify::listener=debug, sophis_notify::subscription::single=debug, sophis_mining::monitor=debug, sophis_testing_integration::subscribe_benchmarks=trace",
    );
    // As we log the panic, we want to set it up after the logger
    sophis_core::panic::configure_panic();
...
async fn utxos_changed_subscriptions_client(address_cycle_seconds: u64, address_max_cycles: usize) {
    ...
    // Setup
    //
    let prealloc_address = Address::new(NetworkType::Simnet.into(), sophis_addresses::Version::PubKeyDilithium, &[1u8; 32]);
    let spk = pay_to_address_script(&prealloc_address);
    // Use a placeholder seed hex for the daemon args (Dilithium wallet integration pending)
    let prealloc_seed_hex = "0101010101010101010101010101010101010101010101010101010101010101".to_string();
    ...
    let txs = common::utils::generate_tx_dag(utxoset.clone(), spk, (TX_COUNT + TX_LEVEL_WIDTH - 1) / TX_LEVEL_WIDTH, TX_LEVEL_WIDTH);
...
/// `cargo test --package sophis-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_a --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package sophis-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_b --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package sophis-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_c --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package sophis-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_d --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package sophis-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_e --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
