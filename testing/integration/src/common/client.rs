...
use sophis_notify::{events::EventType, scope::Scope, subscription::Command};
...
impl ListeningClient {
    pub async fn connect(sophisd: &Daemon) -> Self {
        let client = sophisd.new_multi_listener_client().await;
