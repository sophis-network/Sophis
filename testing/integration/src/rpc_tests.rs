...
use sophis_consensus::params::SIMNET_GENESIS;
use sophis_consensus_core::{constants::MAX_SOMPI, header::Header, subnets::SubnetworkId, tx::Transaction};
use sophis_core::{assert_match, info};
use sophis_grpc_core::ops::SophisdPayloadOps;
...
use sophis_rpc_core::{Notification, api::rpc::RpcApi, model::*};
use sophis_utils::{fd_budget, networking::ContextualNetAddress};
...
/// `cargo test --release --package sophis-testing-integration --lib -- rpc_tests::sanity_test`
#[tokio::test]
async fn sanity_test() {
    sophis_core::log::try_init_logger("info");
    // As we log the panic, we want to set it up after the logger
    sophis_core::panic::configure_panic();
    ...
    for op in SophisdPayloadOps::iter() {
        ...
        let task: JoinHandle<()> = match op {
                    ...
                    let GetBlockTemplateResponse { block, is_synced } = rpc_client
                        .get_block_template_call(
                            ...
                            GetBlockTemplateRequest {
                                pay_address: Address::new(Prefix::Simnet, Version::ScriptHash, &[0u8; 32]),
                                extra_data: Vec::new(),
                            },
                        )
                        .await
            ...
            SophisdPayloadOps::GetBlockTemplate => {
                tst!(op, "see SubmitBlock")
            }

            SophisdPayloadOps::GetCurrentBlockColor => {
                tst!(op, "see SubmitBlock")
            }

            SophisdPayloadOps::GetCurrentNetwork => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SophisdPayloadOps::GetInfo => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client.get_info_call(None, GetInfoRequest {}).await.unwrap();
                    assert_eq!(response.server_version, sophis_core::sophisd_env::version().to_string());
            ...
            SophisdPayloadOps::GetMempoolEntries => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client
            ...
            SophisdPayloadOps::GetConnectedPeerInfo => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SophisdPayloadOps::Unban => {
                tst!(op, "see Ban")
            }

            SophisdPayloadOps::SubmitTransaction => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SophisdPayloadOps::SubmitTransactionReplacement => {
                let rpc_client = client.clone();
                tst!(op, {
                    // Build an erroneous transaction...
                    let transaction = Transaction::new(0, vec![], vec![], 0, SubnetworkId::default(), 0, vec![]);
            ...
            SophisdPayloadOps::GetSubnetwork => {
                let rpc_client = client.clone();
                tst!(op, {
                    let result =
            ...
            SophisdPayloadOps::GetVirtualChainFromBlock => {
                tst!(op, "see SubmitBlock")
            }

            SophisdPayloadOps::GetBlockCount => {
                tst!(op, "see SubmitBlock")
            }

            SophisdPayloadOps::GetBlockDagInfo => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SophisdPayloadOps::ResolveFinalityConflict => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response_result = rpc_client
            ...
            SophisdPayloadOps::GetUtxosByAddresses => {
                let rpc_client = client.clone();
                tst!(op, {
                    let addresses = vec![Address::new(Prefix::Simnet, Version::ScriptHash, &[0u8; 32])];
                    let response =
            ...
            SophisdPayloadOps::GetBalanceByAddress => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client
                        .get_balance_by_address_call(
                            None,
                            GetBalanceByAddressRequest { address: Address::new(Prefix::Simnet, Version::ScriptHash, &[0u8; 32]) },
                        )
                        .await
                        .unwrap();
            ...
            SophisdPayloadOps::GetBalancesByAddresses => {
                let rpc_client = client.clone();
                tst!(op, {
                    let addresses = vec![Address::new(Prefix::Simnet, Version::ScriptHash, &[1u8; 32])];
                    let response = rpc_client
            ...
            SophisdPayloadOps::GetSinkBlueScore => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client.get_sink_blue_score_call(None, GetSinkBlueScoreRequest {}).await.unwrap();
            ...
            SophisdPayloadOps::EstimateNetworkHashesPerSecond => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response_result = rpc_client
            ...
            SophisdPayloadOps::GetMempoolEntriesByAddresses => {
                let rpc_client = client.clone();
                tst!(op, {
                    let addresses = vec![Address::new(Prefix::Simnet, Version::ScriptHash, &[0u8; 32])];
                    let response = rpc_client
            ...
            SophisdPayloadOps::GetCoinSupply => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SophisdPayloadOps::GetConnections => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SophisdPayloadOps::GetSystemInfo => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SophisdPayloadOps::GetServerInfo => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SophisdPayloadOps::GetSyncStatus => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SophisdPayloadOps::GetDaaScoreTimestampEstimate => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SophisdPayloadOps::GetFeeEstimate => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SophisdPayloadOps::GetFeeEstimateExperimental => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            SophisdPayloadOps::GetUtxoReturnAddress => {
                let rpc_client = client.clone();
                tst!(op, {
                    ...
                    assert!(results.is_err_and(|err| {
                        match err {
                            sophis_rpc_core::RpcError::General(msg) => {
            ...
            SophisdPayloadOps::GetVirtualChainFromBlockV2 => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client
            ...
            SophisdPayloadOps::NotifyBlockAdded => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            SophisdPayloadOps::NotifyNewBlockTemplate => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            SophisdPayloadOps::NotifyFinalityConflict => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            SophisdPayloadOps::NotifyUtxosChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            SophisdPayloadOps::NotifySinkBlueScoreChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            SophisdPayloadOps::NotifyPruningPointUtxoSetOverride => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            SophisdPayloadOps::NotifyVirtualDaaScoreChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            SophisdPayloadOps::NotifyVirtualChainChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
                    rpc_client
            ...
            SophisdPayloadOps::StopNotifyingUtxosChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            SophisdPayloadOps::StopNotifyingPruningPointUtxoSetOverride => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
