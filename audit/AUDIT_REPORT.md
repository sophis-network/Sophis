...
### Findings ledger (final state, 24 total — updated Session 11, 2026-05-15; + F-25 post-audit soak 2026-05-17; + F-26 DA-store-no-pruning code gap, P1 pre-mainnet, 2026-05-18)
...
| F-25 | test-methodology → see F-26 | 🔁 **SUPERSEDED on root-cause + fix by F-26 (2026-05-18).** The real cause is **NOT** "pruning_depth too deep for a short soak" — it is that the **self-DA carrier store has NO pruning implementation at all** (F-26): the pruning_processor never references it, `DbDaStore` has no delete path, design §6.1/§10.7 is unimplemented → the DA store grows unbounded **forever**, not "~30 h then pruned". Consequently the **"devnet `pruning_depth` override = validated fix" claim in this row is RETRACTED** — lowering `pruning_depth` would not bound the DA store (it is not wired into pruning at any depth). What still stands as valid evidence: the A/B (mimalloc exonerated), the full-build bisect (no post-Friday regression), and the 1-node 4 h unbounded-growth measurement. Only the root-cause attribution and the proposed fix were wrong; historical investigation trail retained verbatim below. —— ⚠️ **(original 2026-05-17 framing, now superseded:) soak harness defect — not a Phase 6 code defect; root cause stated as: devnet `pruning_depth` unreachable in any short soak → unbounded store growth → commit exhaustion.** `pruning_depth = max(lower_bound≈584k, BPS×PRUNING_DURATION = 10×108_000) = 1,080,000 blocks` (`bps.rs:96`, `constants.rs`). Pruning does not advance until the DAG is ~1.08M blocks deep; **time-to-first-prune = PRUNING_DURATION = 108_000 s ≈ 30 h, BPS-independent, and identical on mainnet/testnet/devnet (all 10 BPS)** — by design (Kaspa prunality). So any <~30 h soak **never prunes**; store grows 100% unbounded (~120 MB/min/node; 1,398 SST; ~10 GB/node at 83 min). Confirmed: **0 pruning log lines** in 83 min. **A/B (single-variable, 2026-05-17) ruled OUT the `d14bd71` mimalloc 0.1.48→0.1.50 bump:** per-node committed memory = **datadir + ~1.0–1.4 GB RocksDB overhead, identical** with default vs aggressively-tuned allocator (`ARENA_EAGER_COMMIT=0;PURGE_DELAY=0`); allocator is *not* the driver (and `PURGE_DELAY=0` halves throughput for zero memory gain — do not use). So commit ≈ store size; the killer is store growth, not RAM and not the allocator (supersedes earlier wrong "needs 64 GB / pagefile-masking" framing — both retracted). **Independently corroborated by a full-build bisect (2026-05-17): the Friday-morning commit `5b22309` (pre the 55-dep bump / kHeavyHash eradication / 114-dep removal / rust bump / mimalloc 0.1.50) built in an isolated worktree and run with the *exact* Stage-2 12h config died at ~86 min — same mode (NODE_DIED, commit-exhaust), 0 daemon panics, 0 DA-index errors, 68 GB datadir, 8.6k carriers / 1.83 GB — statistically identical to HEAD's ~83 min. ⇒ NO post-Friday regression; root cause is structural and build-invariant.** **RESOLVED by direct measurement (1-node 4h instrumented run, 2026-05-18):** per-node **Private does NOT plateau**. A single node, clean 240-min run, climbed monotonically to **19.1 GB committed** (datadir 21.7 GB, RSS plateaued ~3.7 GB), slope decelerating 260→~25 MB/min by 180 min then **re-accelerating to ~75 MB/min at 4 h — no asymptote**. ⇒ per-node committed memory grows **unbounded with the store** over hours (not the earlier "≈datadir 1:1" detail — datadir runs ~2.6 GB ahead — but unbounded nonetheless). This **explains the 5-node ~85-min death** (5 co-located nodes each on this curve collectively exhaust the ~63 GB commit limit) and means the soak **cannot reach 12 h on any single machine without bounding the store**. Stage-1's clean 4 h with 5 nodes is therefore **still anomalous and unresolved**: the only explanation consistent with this measured curve is that Stage-1's box had a far larger CommitLimit/pagefile at that time — **inferential, never measured, explicitly not claimed as proven**; it is the lone residual, not the central question. Earlier "plateau / co-located-footprint-only / environmental" hopes were **overclaims, retracted**. The pruning-override fix is now **validated by 4 h of direct single-node evidence**, not theory. **Partial run CLEAN: 9,417 carriers / 2.04 GB / 24,626 blocks / 0 daemon panics / 0 DA-index errors.** Also *why* G7/G8 are perpetually deferred — pruning is structurally unreachable here. Fix has **two distinct parts**: (1) **devnet-only `pruning_depth` soak override** → bounded soak + G7/G8 finally testable — but this validates only the pruning *mechanism*, NOT mainnet footprint; (2) **separately**, mainnet/testnet inherently grow unbounded for their first ~30 h by design — that window must be **characterized at realistic load with real params** (operational sizing, not a soak knob). | **No** — soak-harness param; no Phase 6 correctness defect; verdict unchanged |
| F-26 | **P1 — pre-mainnet; CODE RESOLVED, verdict re-review pending** | ✅ **RESOLVED IN CODE 2026-05-18 — Fix A + Fix B implemented & validated** (clearing the MAINNET gate now needs only a founder/SIP verdict re-review). Branch `f26-da-pruning`: `098333c` M1 (`prune_block_batch` + wired into `pruning_processor`), `143bf7e` M2 (deterministic integration test = **G8** prune-correctness), `adcd024` M3.1 (metadata/body store split — storage-only refinement, founder-approved), `c769b43` M3.2 (short body-horizon GC). Validation: **unit 12/12** (`da.rs`); **G8** the real pruning processor invokes `prune_block_batch` and removes pruned-block carriers from every sub-store while recent survive (`da_carrier_prune_test`, no regression across A+split+GC); **G7** real private process **kill-9 + restart** on a persisted DA-pruned datadir → clean recovery, chain advanced +2.3k blocks, pruning resumed (PASS). Fix A bounds the DA store to `pruning_depth` (closes the unbounded-forever gap); Fix B further shrinks the bounded body window ~30 h→~6 h (non-consensus — H1; `--archival` opts out). Spec/impl: `oracle/docs/PHASE6_DA_PRUNING_SIP_DRAFT.md`. G7/G8 no longer deferred — both proven. The discovery-record analysis below is retained verbatim. —— ⚠️ **(discovery record, 2026-05-18:) OPEN — real code gap, NOT a soak/test artifact.** Discovered 2026-05-18 via Layer-A code validation of the F-25 soak. **The self-DA carrier store (prefixes 196-199) has NO pruning/deletion implementation anywhere.** Airtight evidence: (a) `pipeline/pruning_processor/processor.rs` references `da_store`/`DbDaStore` **0 times** and prunes a fixed enumerated store set (utxo_multisets, utxo_diffs, acceptance_data, block_transactions, ghostdag, daa_excluded, tips, selected_chain, relations, reachability, statuses, headers) — **DA store absent**; (b) `DbDaStore` (`consensus/src/model/stores/da.rs`) has **no delete/prune fn** — only `index_carrier_batch` (insert-only); even the J4 events store implements `by_block.delete`, the DA store does not; (c) `consensus/src/consensus/storage.rs` builds `da_store` but never wires it into the pruning path. ⇒ `PHASE6_DA_DESIGN.md` §6.1/§10.7 ("carrier bodies follow the same pruning regime as transactions / pruned nodes drop bodies") is **specified but unimplemented** — doc-vs-code drift on a load-bearing property. Effect: every node's DA store grows **unbounded forever** (insert-only), on **testnet AND mainnet** — not a ~30 h window, ∞. The design's own **T11 storage-griefing mitigation ("pruned nodes drop bodies") does not exist**; only the fee market remains. Layer-A **H1 confirmed**: consensus validation is **body-free** (`SophisDaBackend::verify_payload`/`verify_bundle` use only metadata `blue_score`/`fragment_count`/bundle membership; rollup `verify_batch` uses journal + sequencer Dilithium sig — no carrier body read) → the fix is safe to design. Fix design: see note below. | **Code resolved** (branch f26-da-pruning, 4 commits, validated); clearing the MAINNET gate = founder/SIP verdict re-review |
...
**Post-audit soak (F-25, 2026-05-17 — test-methodology, verdict unchanged):** A Stage-2 12h soak (config = exact Stage-1 mirror) ran clean **~83 min** then commit-exhausted on the 15.6 GB Windows box. **No Phase 6 correctness defect** (0 daemon panics, 0 DA-index errors, 9,417 carriers / 2.04 GB; G2/G6 auto-FAILs are the documented dead-node / ±20 % measurement artifacts). **Root cause (final, evidence-backed):** devnet `pruning_depth = 1,080,000 blocks` (`bps.rs:96`; 10 BPS × 108_000 s); time-to-first-prune = **~30 h, BPS-independent, identical on all three networks by design**. Any <~30 h soak never prunes → store grows unbounded → committed memory (= datadir + ~1.2 GB RocksDB overhead) exhausts commit. A **single-variable A/B** ruled out the `d14bd71` mimalloc 0.1.48→0.1.50 bump (per-store overhead identical default vs tuned allocator). Earlier "needs 64 GB / pagefile-masking" framings were wrong and are **retracted**; the Stage-1-vs-Stage-2 survival delta is commit-headroom variance, secondary. **SUPERSEDED 2026-05-18 → F-26.** The "two-part fix (devnet `pruning_depth` override …)" previously stated here is **RETRACTED**: Layer-A code validation found the self-DA carrier store has **no pruning implementation at all** (F-26) — lowering `pruning_depth` cannot bound a store that is never wired into pruning. Real root cause = F-26 (DA store insert-only, design §6.1/§10.7 unimplemented, unbounded forever on mainnet+testnet). **Verdict impact:** TESTNET ✅ still stands (testnet is precisely where such gaps surface — and this one was found pre-testnet). **MAINNET approval is NO LONGER unconditional** — F-26 is a **P1 pre-mainnet blocker** (unbounded per-node disk, design-stated retention unimplemented); mainnet is **gated on F-26 resolution**, pending founder/SIP verdict review. Fix design (analysis, pre-code): **Fix A (mandatory, closes F-26)** — wire `DbDaStore` into the pruning processor: add a `prune_block(block_hash)` path that, via the existing `by_block` index (block→payload_ids), deletes the payloads + de-references them from `bundles`/`by_domain`, called from the per-block prune loop alongside `block_transactions_store.delete_batch`; bounds the DA store to `pruning_depth` (the design's stated intent). **Fix B (optional, on top of A — the "Option 2" short body horizon)** — split `PayloadEntry` so the heavy `script` body drops on a short horizon while light metadata (`blue_score`/`fragment_count`/`bundle_id`/`domain_byte`/`accepting_block_hash`/`fragment_index`) is kept to `pruning_depth`; safe because consensus is body-free (H1); reduces the bounded ceiling from ~flood×30 h to ~tens of GB. Both are consensus/DA-store-ABI changes → SIP/hard-fork-class; design only until founder/SIP ratifies. G7/G8 remain deferred (and are untestable until Fix A exists). **RESOLVED 2026-05-18 (M1–M3.2):** Fix A + Fix B implemented on branch `f26-da-pruning` (`098333c`/`143bf7e`/`adcd024`/`c769b43`) and validated — unit 12/12, deterministic integration **G8** + real-restart **G7** both PASS, no regression. The Fix-B refinement actually adopted = a **storage-only split** (`PayloadEntry` stays the public type; `PayloadMeta`/196 + `PayloadBody`/209 internal) — ~1/5 the blast radius of the literal SIP "PayloadEntry loses script", founder-approved; zero change to virtual_processor/svm_da/rpc/consensusmanager. F-26 is now **code-resolved**; the MAINNET re-gating is pending only a **founder/SIP verdict re-review** (evidence = this section + `oracle/docs/PHASE6_DA_PRUNING_SIP_DRAFT.md`). G7/G8 **no longer deferred** — both proven (G8 deterministic integration; G7 real private kill-9 + restart, clean).
