...
// ── MerkleHash ──────────────────────────────────────────────────────────────
// 48-byte (384-bit) hash used exclusively for Merkle tree nodes.
// blake2b-384 gives exactly 2^128 quantum collision resistance (NIST Level 1)
// via the BHT algorithm: 2^(384/3) = 2^128.
// Using a separate type prevents accidental use of Merkle roots where a
// block-hash or tx-hash is expected.

pub const MERKLE_HASH_SIZE: usize = 48;

// Note: serde macros require array sizes ≤ 32; MerkleHash(48) needs manual impls.
#[derive(Eq, Clone, Copy, BorshSerialize, BorshDeserialize)]
pub struct MerkleHash([u8; MERKLE_HASH_SIZE]);

impl Default for MerkleHash {
    fn default() -> Self {
        MerkleHash([0u8; MERKLE_HASH_SIZE])
    }
}

impl serde::Serialize for MerkleHash {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        if serializer.is_human_readable() { serializer.serialize_str(&self.to_string()) } else { serializer.serialize_bytes(&self.0) }
    }
}
...
impl<'de> serde::Deserialize<'de> for MerkleHash {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        if deserializer.is_human_readable() {
            ...
            MerkleHash::from_str(s).map_err(D::Error::custom)
        } else {
            let bytes = <&[u8]>::deserialize(deserializer)?;
            MerkleHash::try_from_slice(bytes).map_err(D::Error::custom)
        }
    }
}
...
impl MerkleHash {
    #[inline(always)]
    pub const fn from_bytes(bytes: [u8; MERKLE_HASH_SIZE]) -> Self {
        MerkleHash(bytes)
    }

    #[inline(always)]
    pub const fn as_bytes(self) -> [u8; MERKLE_HASH_SIZE] {
        self.0
    }

    #[inline(always)]
    pub fn from_slice(bytes: &[u8]) -> Self {
        Self(<[u8; MERKLE_HASH_SIZE]>::try_from(bytes).expect("Slice must have the length of MerkleHash"))
    }

    #[inline(always)]
    pub fn try_from_slice(bytes: &[u8]) -> Result<Self, TryFromSliceError> {
        Ok(Self(<[u8; MERKLE_HASH_SIZE]>::try_from(bytes)?))
    }
}

impl PartialEq for MerkleHash {
    #[inline(always)]
    fn eq(&self, other: &Self) -> bool {
        self.0 == other.0
    }
...
impl StdHash for MerkleHash {
    #[inline(always)]
    fn hash<H: StdHasher>(&self, state: &mut H) {
        self.0.chunks_exact(8).for_each(|c| u64::from_le_bytes(c.try_into().unwrap()).hash(state));
    }
}

impl Display for MerkleHash {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let mut hex = [0u8; MERKLE_HASH_SIZE * 2];
        faster_hex::hex_encode(&self.0, &mut hex).expect("The output is exactly twice the size of the input");
        f.write_str(unsafe { str::from_utf8_unchecked(&hex) })
    }
}
...
impl From<[u8; MERKLE_HASH_SIZE]> for MerkleHash {
    fn from(value: [u8; MERKLE_HASH_SIZE]) -> Self {
        MerkleHash(value)
    }
}

impl TryFrom<&[u8]> for MerkleHash {
    type Error = TryFromSliceError;
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        MerkleHash::try_from_slice(value)
    }
}

impl TryFrom<Vec<u8>> for MerkleHash {
    type Error = Vec<u8>;
    fn try_from(value: Vec<u8>) -> Result<Self, Self::Error> {
        <[u8; MERKLE_HASH_SIZE]>::try_from(value).map(MerkleHash)
    }
}

impl From<MerkleHash> for Vec<u8> {
    fn from(value: MerkleHash) -> Self {
        value.0.to_vec()
    }
}

impl AsRef<[u8; MERKLE_HASH_SIZE]> for MerkleHash {
    fn as_ref(&self) -> &[u8; MERKLE_HASH_SIZE] {
        &self.0
    }
}
...
impl ToHex for MerkleHash {
    fn to_hex(&self) -> String {
        self.to_string()
    }
}

impl FromHex for MerkleHash {
    type Error = faster_hex::Error;
    fn from_hex(hex_str: &str) -> Result<Self, Self::Error> {
        Self::from_str(hex_str)
    }
...
impl MemSizeEstimator for MerkleHash {}

pub const ZERO_MERKLE_HASH: MerkleHash = MerkleHash([0; MERKLE_HASH_SIZE]);
