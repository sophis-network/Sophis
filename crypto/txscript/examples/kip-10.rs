...
use sophis_txscript_errors::TxScriptError::{EvalFalse, VerifyError};

/// Main function to execute all Sophis transaction script scenarios.
///
/// # Returns
///
...
///
/// This scenario demonstrates the use of custom opcodes and script execution within the Sophis blockchain ecosystem.
...
fn threshold_scenario() -> ScriptBuilderResult<()> {
    ...
    // Check owner branch
    {
        ...
        let sig_hash = calc_signature_hash(&tx.as_verifiable(), 0, SIG_HASH_ALL, &reused_values);
...
fn p2pk_as_vec(owner: &Keypair) -> Vec<u8> {
    let p2pk = pay_to_address_script(&Address::new(
        Prefix::Mainnet,
        Version::PubKeyDilithium,
        owner.x_only_public_key().0.serialize().as_slice(),
    ));
...
///
/// This scenario demonstrates the use of a shared secret within the Sophis blockchain ecosystem.
