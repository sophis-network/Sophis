...
use sophis_consensus_core::hashing::sighash::{SigHashReusedValues, SigHashReusedValuesUnsync, calc_signature_hash};
use sophis_consensus_core::hashing::sighash_type::SigHashType;
use sophis_consensus_core::tx::{PopulatedTransaction, ScriptPublicKey, TransactionInput, UtxoEntry, VerifiableTransaction};
use sophis_txscript_errors::TxScriptError;
// Dilithium-2 (ML-DSA-44) sizes per FIPS 204 §5 — constants are private in libcrux
const DILITHIUM2_VK_SIZE: usize = 1312;
...
pub const MAX_SCRIPT_ELEMENT_SIZE: usize = 4096; // increased for Dilithium-2 signatures (~2420 bytes) and redeem scripts (~1316 bytes)
...
enum Signature {
    // Compact repr: first 32 bytes of the 2420-byte Dilithium signature.
    // Collision probability is negligible for a performance cache.
    Dilithium([u8; 32]),
}

#[derive(Clone, Hash, PartialEq, Eq)]
enum PublicKey {
    // Compact repr: first 32 bytes of the 1312-byte Dilithium verification key.
    Dilithium([u8; 32]),
}
...
impl<'a, T: VerifiableTransaction, Reused: SigHashReusedValues> TxScriptEngine<'a, T, Reused> {
    ...
    pub fn execute_opcode(&mut self, opcode: DynOpcodeImplementation<T, Reused>) -> Result<(), TxScriptError> {
        ...
        // Different from sophisd: Illegal and disabled opcode are checked on execute instead
        // Note that this includes OP_RESERVED which counts as a push operation.
    ...
    // ---------------------------------------------------------------------------
    // Dilithium-2 (ML-DSA-44, FIPS 204) — post-quantum signature verification
    //
    ...
    fn check_dilithium_signature(&mut self, hash_type: SigHashType, key: &[u8], sig: &[u8]) -> Result<bool, TxScriptError> {
        ...
        match self.script_source {
            ScriptSource::TxInput { tx, idx, .. } => {
                if sig.len() != DILITHIUM2_SIG_SIZE {
                    return Err(TxScriptError::SigLength(sig.len()));
                }
                if key.len() != DILITHIUM2_VK_SIZE {
                    return Err(TxScriptError::PubKeyFormat);
                }

                // Compute the transaction signature hash (Blake2b of TX) as the Dilithium message
                let sig_hash = calc_signature_hash(tx, idx, hash_type, self.reused_values);
                let message: &[u8] = &sig_hash.as_bytes()[..];

                // Build compact cache key: first 32 bytes of VK + first 32 bytes of sig + message
                let mut vk_key = [0u8; 32];
                let mut sig_key = [0u8; 32];
                let mut msg_key = [0u8; 32];
                vk_key.copy_from_slice(&key[..32]);
                sig_key.copy_from_slice(&sig[..32]);
                msg_key.copy_from_slice(message);
                let cache_key =
                    SigCacheKey { signature: Signature::Dilithium(sig_key), pub_key: PublicKey::Dilithium(vk_key), message: msg_key };

                // Check cache first
                if let Some(cached) = self.sig_cache.get(&cache_key) {
                    trace!(target: "txscript", "dilithium sig cache hit");
                    return Ok(cached);
                }

                let vk_bytes: [u8; DILITHIUM2_VK_SIZE] = key.try_into().map_err(|_| TxScriptError::PubKeyFormat)?;
                let sig_bytes: [u8; DILITHIUM2_SIG_SIZE] = sig.try_into().map_err(|_| TxScriptError::SigLength(sig.len()))?;

                let vk = MLDSA44VerificationKey::new(vk_bytes);
                let signature = MLDSA44Signature::new(sig_bytes);

                // context string is empty — FIPS 204 standard convention
                let valid = match ml_dsa_44::verify(&vk, message, b"", &signature) {
                    Ok(()) => true,
                    Err(_) => false,
                };
...
mod bitcoind_tests {
    ...
    use sophis_consensus_core::constants::MAX_TX_IN_SEQUENCE_NUM;
    use sophis_consensus_core::hashing::sighash::SigHashReusedValuesUnsync;
    use sophis_consensus_core::tx::{
        PopulatedTransaction, ScriptPublicKey, Transaction, TransactionId, TransactionOutpoint, TransactionOutput,
    };
    ...
    // ---------------------------------------------------------------------------
    // Dilithium-2 (ML-DSA-44) sign + verify integration test
    // ---------------------------------------------------------------------------
    #[test]
    fn test_dilithium_sign_and_verify() {
        ...
        use sophis_consensus_core::hashing::sighash_type::SIG_HASH_ALL;
        ...
        // 1. Generate Dilithium-2 keypair
        ...
        // 2. Build a minimal TX with one input
        let prev_id = TransactionId::default();
        let script_pub_key = crate::standard::pay_to_address_script(&sophis_addresses::Address::new(
            sophis_addresses::Prefix::Devnet,
            sophis_addresses::Version::PubKeyDilithium,
            // address payload = Blake2b hash of the redeem script (simplified: use vk hash)
            blake2b_simd::Params::new().hash_length(32).to_state().update(vk.as_ref()).finalize().as_bytes(),
        ));

        let tx = Transaction::new(
            0,
            vec![TransactionInput {
                previous_outpoint: TransactionOutpoint { transaction_id: prev_id, index: 0 },
                signature_script: vec![],
                sequence: 0,
                sig_op_count: 1,
            }],
            vec![TransactionOutput { value: 1000, script_public_key: script_pub_key.clone() }],
            0,
            SUBNETWORK_ID_NATIVE,
            0,
            vec![],
        );
        let utxo = UtxoEntry { amount: 5000, script_public_key: script_pub_key, block_daa_score: 0, is_coinbase: false };
        let populated = PopulatedTransaction::new(&tx, vec![utxo]);

        // 3. Compute sighash and sign with Dilithium-2
        let reused = sophis_consensus_core::hashing::sighash::SigHashReusedValuesUnsync::new();
        let sig_hash = sophis_consensus_core::hashing::sighash::calc_signature_hash(&populated, 0, SIG_HASH_ALL, &reused);
        let message = &sig_hash.as_bytes()[..];
        let randomness: [u8; SIGNING_RANDOMNESS_SIZE] = [7u8; SIGNING_RANDOMNESS_SIZE];
        let signature = ml_dsa_44::sign(&sk, message, b"", randomness).expect("Dilithium sign should not fail");

        // 4. Verify directly
        let result = ml_dsa_44::verify(&vk, message, b"", &signature);
        assert!(result.is_ok(), "Dilithium signature should verify correctly");

        // 5. Verify with wrong message should fail
        let wrong_message = &[0u8; 32][..];
        let bad_result = ml_dsa_44::verify(&vk, wrong_message, b"", &signature);
        assert!(bad_result.is_err(), "Dilithium should reject wrong message");
    }
}
