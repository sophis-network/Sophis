...
use sophis_consensus_core::tx::{ScriptPublicKey, ScriptPublicKeyVersion};
use std::{
...
impl ScriptClass {
    pub fn from_script(script_public_key: &ScriptPublicKey) -> Self {
        ...
        if script_public_key.version() == MAX_SCRIPT_PUBLIC_KEY_VERSION {
            if Self::is_pay_to_script_hash(script_public_key_) { Self::ScriptHash } else { ScriptClass::NonStandard }
        } else {
...
impl From<Version> for ScriptClass {
    fn from(value: Version) -> Self {
        match value {
            Version::PubKeyDilithium => ScriptClass::ScriptHash,
...
#[cfg(test)]
mod tests {
    use sophis_consensus_core::tx::ScriptVec;
