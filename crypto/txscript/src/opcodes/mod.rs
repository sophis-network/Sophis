...
use sophis_consensus_core::hashing::sighash::SigHashReusedValues;
...
opcode_list! {
    ...
    opcode OpIf<0x63, 1>(self, vm) {
        ...
        if vm.is_executing() {
            ...
            // the similar flow of go-sophisd
    ...
    // Undefined opcodes
    // Post-quantum signature verification — CRYSTALS-Dilithium-2 (ML-DSA-44, FIPS 204)
    // Stack: <sig: 2420 bytes> <pubkey: 1312 bytes> OP_CHECKSIG_DILITHIUM
    opcode OpCheckSigDilithium<0xc4, 1>(self, vm) {
        let [mut sig, key] = vm.dstack.pop_raw()?;
        match sig.pop() {
            Some(typ) => {
                let hash_type = SigHashType::from_u8(typ).map_err(|e| TxScriptError::InvalidSigHashType(typ))?;
                match vm.check_dilithium_signature(hash_type, key.as_slice(), sig.as_slice()) {
                    Ok(valid) => {
...
mod test {
    ...
    use sophis_consensus_core::constants::{SOMPI_PER_SOPHIS, TX_VERSION};
    ...
    #[test]
    #[ignore = "TODO Sophis: test asserts opcode 0xc4 is disabled, but it's now OpCheckSigDilithium (active). Update test vectors for Dilithium-only opcode set."]
    fn test_opcode_invalid() {
        let tests: Vec<Box<dyn OpCodeImplementation<PopulatedTransaction, SigHashReusedValuesUnsync>>> = vec![
            ...
            opcodes::OpCheckSigDilithium::empty().expect("Should accept empty"),
    ...
    mod kip10 {
        ...
        use sophis_consensus_core::tx::MutableTransaction;
        ...
        fn create_mock_spk(value: u8) -> ScriptPublicKey {
            ...
            let addr = Address::new(Prefix::Testnet, Version::PubKeyDilithium, &pub_key);
