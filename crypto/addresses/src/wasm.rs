...
mod tests {
    ...
    // Sophis PubKeyDilithium address derived from the 32-byte payload below.
    // Generated via `Address::new(Prefix::Mainnet, Version::PubKeyDilithium, &payload)`.
    const TEST_ADDRESS: &str = "sophis:qfaupqedugjte8jvakdp8ks540mwu3rk8mzza762gsdrry864drfjx3f3k55m";
    const TEST_PAYLOAD: &str = "qfaupqedugjte8jvakdp8ks540mwu3rk8mzza762gsdrry864drfjx3f3k55m";

    #[wasm_bindgen_test]
    fn test_wasm_serde_constructor() {
        let a = Address::constructor(TEST_ADDRESS);
        let value = to_value(&a).unwrap();

        assert_eq!(JsValue::from_str("string"), value.js_typeof());
        assert_eq!(value, JsValue::from_str(TEST_ADDRESS));
        assert_eq!(a, from_value(value).unwrap());
    }

    #[wasm_bindgen_test]
    fn test_wasm_js_serde_object() {
        let expected = Address::constructor(TEST_ADDRESS);

        let obj = Object::new();
        obj.set("version", &JsValue::from_str("PubKeyDilithium")).unwrap();
        obj.set("prefix", &JsValue::from_str("sophis")).unwrap();
        obj.set("payload", &JsValue::from_str(TEST_PAYLOAD)).unwrap();
