use crate::imports::*;
use sophis_daemon::SophisdConfig;
...
pub struct Node {
    settings: SettingsStore<SophisdSettings>,
    mute: Arc<AtomicBool>,
...
impl Default for Node {
    fn default() -> Self {
        Node {
            settings: SettingsStore::try_new("sophisd").expect("Failed to create node settings store"),
...
impl Handler for Node {
    fn verb(&self, ctx: &Arc<dyn Context>) -> Option<&'static str> {
        if let Ok(ctx) = ctx.clone().downcast_arc::<SophisCli>() {
            ctx.daemons().clone().sophisd.as_ref().map(|_| "node")
        } else {
            None
        }
    }
    ...
    fn help(&self, _ctx: &Arc<dyn Context>) -> &'static str {
        "Manage the local Sophis node instance"
    }

    async fn start(self: Arc<Self>, _ctx: &Arc<dyn Context>) -> cli::Result<()> {
        self.settings.try_load().await.ok();
        if let Some(mute) = self.settings.get(SophisdSettings::Mute) {
    ...
    async fn handle(self: Arc<Self>, ctx: &Arc<dyn Context>, argv: Vec<String>, cmd: &str) -> cli::Result<()> {
        let ctx = ctx.clone().downcast_arc::<SophisCli>()?;
...
impl Node {
    ...
    async fn create_config(&self, ctx: &Arc<SophisCli>) -> Result<SophisdConfig> {
        let location: String = self
            .settings
            .get(SophisdSettings::Location)
        ...
        let mute = false;
        let config = SophisdConfig::new(location.as_str(), network_id, mute);
        Ok(config)
    }

    async fn main(self: Arc<Self>, ctx: Arc<SophisCli>, mut argv: Vec<String>, cmd: &str) -> Result<()> {
        if argv.is_empty() {
            return self.display_help(ctx, argv).await;
        }
        let sophisd = ctx.daemons().sophisd();
        match argv.remove(0).as_str() {
            "start" => {
                let mute = self.mute.load(Ordering::SeqCst);
                if mute {
                    tprintln!(ctx, "starting sophis node... {}", style("(logs are muted, use 'node mute' to toggle)").dim());
                } else {
                    tprintln!(ctx, "starting sophis node... {}", style("(use 'node mute' to mute logging)").dim());
                }

                let wrpc_client = ctx.wallet().try_wrpc_client().ok_or(Error::custom("Unable to start node with non-wRPC client"))?;

                sophisd.configure(self.create_config(&ctx).await?).await?;
                sophisd.start().await?;
            ...
            "mute" | "logs" => {
                ...
                // sophisd.mute(mute).await?;
                self.settings.set(SophisdSettings::Mute, mute).await?;
            }
            "status" => {
                let status = sophisd.status().await?;
                tprintln!(ctx, "{}", status);
            }
            "select" => {
            ...
            "version" => {
                sophisd.configure(self.create_config(&ctx).await?).await?;
                let version = sophisd.version().await?;
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<SophisCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ("select", "Select Sophisd executable (binary) location"),
                ("version", "Display Sophisd executable version"),
                ("start", "Start the local Sophis node instance"),
                ("stop", "Stop the local Sophis node instance"),
                ("restart", "Restart the local Sophis node instance"),
                ("kill", "Kill the local Sophis node instance"),
                ("status", "Get the status of the local Sophis node instance"),
    ...
    async fn select(self: Arc<Self>, ctx: Arc<SophisCli>, path: Option<String>) -> Result<()> {
        let root = nw_sys::app::folder();

        match path {
            None => {
                let binaries = sophis_daemon::locate_binaries(root.as_str(), "sophisd").await?;

                if binaries.is_empty() {
                    tprintln!(ctx, "No sophisd binaries found");
                } else {
                    let binaries = binaries.iter().map(|p| p.display().to_string()).collect::<Vec<_>>();
                    if let Some(selection) = ctx.term().select("Please select a sophisd binary", &binaries).await? {
                        tprintln!(ctx, "selecting: {}", selection);
                        self.settings.set(SophisdSettings::Location, selection.as_str()).await?;
                    } else {
            ...
            Some(path) => {
                if fs::exists(&path).await? {
                    ...
                    self.settings.set(SophisdSettings::Location, path.as_str()).await?;
                } else {
                    twarnln!(ctx, "destination binary not found, please specify full path including the binary name");
                    twarnln!(ctx, "example: 'node select /home/user/testnet/sophisd'");
                    tprintln!(ctx, "no selection is made");
                }
            }
        }
    ...
    pub async fn handle_event(&self, ctx: &Arc<SophisCli>, event: Event) -> Result<()> {
        let term = ctx.term();
        ...
        match event {
            ...
            Event::Exit(_code) => {
                tprintln!(ctx, "Sophisd has exited");
                self.is_running.store(false, Ordering::SeqCst);
                term.refresh_prompt();
            }
            Event::Error(error) => {
                tprintln!(ctx, "{}", style(format!("Sophisd error: {error}")).red());
                self.is_running.store(false, Ordering::SeqCst);
                term.refresh_prompt();
            }
