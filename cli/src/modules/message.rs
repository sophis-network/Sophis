use sophis_addresses::Version;
use sophis_bip32::secp256k1::XOnlyPublicKey;
use sophis_wallet_core::message::SignMessageOptions;
use sophis_wallet_core::{
...
impl Message {
    async fn main(self: Arc<Self>, ctx: Arc<SophisCli>, argv: Vec<String>, _cmd: &str) -> Result<()> {
        if argv.is_empty() {
        ...
        match argv.first().unwrap().as_str() {
            ...
            "verify" => {
                ...
                self.verify(ctx, sophis_address, signature, message).await?;
            }
            v => {
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<SophisCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ("sign <sophis_address>", "Sign a message with the private key that matches the given address. Prompts for message."),
                (
                    "verify <sophis_address> <signature>",
                    "Verify the signature against the message and sophis_address. Prompts for message.",
                ),
            ],
            None,
        )?;
    ...
    async fn sign(self: Arc<Self>, ctx: Arc<SophisCli>, sophis_address: &str, message: &str) -> Result<()> {
        let sophis_address = Address::try_from(sophis_address)?;
        if sophis_address.version != Version::PubKeyDilithium {
        ...
        let privkey = self.get_address_private_key(&ctx, sophis_address).await?;
    ...
    async fn verify(self: Arc<Self>, ctx: Arc<SophisCli>, sophis_address: &str, signature: &str, message: &str) -> Result<()> {
        let sophis_address = Address::try_from(sophis_address)?;
        if sophis_address.version != Version::PubKeyDilithium {
            return Err(Error::custom("Address not supported for message signing. Only supports PubKey addresses"));
        }

        let pubkey = XOnlyPublicKey::from_slice(&sophis_address.payload[0..32]).unwrap();
    ...
    async fn get_address_private_key(self: Arc<Self>, ctx: &Arc<SophisCli>, sophis_address: Address) -> Result<[u8; 32]> {
        ...
        match account.account_kind().as_ref() {
            BIP32_ACCOUNT_KIND => {
                ...
                let (receive, change) = account.derivation().addresses_indexes(&[&sophis_address])?;
