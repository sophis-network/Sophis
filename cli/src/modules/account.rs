use sophis_wallet_core::account::BIP32_ACCOUNT_KIND;
use sophis_wallet_core::account::LEGACY_ACCOUNT_KIND;
use sophis_wallet_core::account::MULTISIG_ACCOUNT_KIND;
        ...
        match action.as_str() {
            ...
            "import" => {
                if argv.is_empty() {
                    ...
                    ctx.term().help(
                        &[
                            ("account import legacy-data", "Import KDX keydata file or sophisnet web wallet data on the same domain"),
                            (
                                "account import mnemonic bip32",
                                "Import Bip32 (12 or 24 word mnemonics used by sophiswallet, kaspium, onekey, tangem etc.)",
                            ),
                            (
                                "account import mnemonic legacy",
                                "Import accounts 12 word mnemonic used by legacy applications (KDX and sophisnet web wallet)",
                            ),
                            (
                ...
                match import_kind.as_ref() {
                    "legacy-data" => {
                        ...
                        if exists_legacy_v0_keydata().await? {
                                ...
                                .import_legacy_keydata(
                                    ...
                                    Some(Arc::new(move |processed: usize, _, balance, txid| {
                                        if let Some(txid) = txid {
                                            tprintln!(
                                                ctx_,
                                                "Scan detected {} SPHS at index {}; transfer txid: {}",
                                                sompi_to_sophis_string(balance),
                                                processed,
                                                txid
                                            );
                                        } else if processed > 0 {
                                            tprintln!(
                                                ctx_,
                                                "Scanned {} derivations, found {} SPHS",
                                                processed,
                                                sompi_to_sophis_string(balance)
                                            );
                                        } else {
                            ...
                            return Err("'sophisnet' web wallet storage not found at this domain name".into());
                        } else {
                    ...
                    "mnemonic" => {
                        if argv.is_empty() {
                            ...
                            tprintln!(ctx, "please use 'legacy' for 12-word KDX and sophisnet web wallet mnemonics\r\n");
                            return Ok(());
                        }
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<SophisCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ...
                (KDX and sophisnet web wallet). Use 'account import' for additional help.",
                ),
            ...
            .derivation_scan(
                ...
                Some(Arc::new(move |processed: usize, _, balance, txid| {
                    if let Some(txid) = txid {
                        ...
                        tprintln!(ctx_, "Scanned {} derivations, found {} SPHS", processed, sompi_to_sophis_string(balance));
                    }
                })),
            )
            .await?;
