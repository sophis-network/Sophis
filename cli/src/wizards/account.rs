use crate::cli::SophisCli;
use crate::imports::*;
use crate::result::Result;
use sophis_bip32::{Language, Mnemonic, WordCount};
use sophis_wallet_core::account::MULTISIG_ACCOUNT_KIND;
use sophis_wallet_core::storage::keydata::PrvKeyDataVariantKind;
// use sophis_wallet_core::runtime::wallet::AccountCreateArgsBip32;
// use sophis_wallet_core::runtime::{PrvKeyDataArgs, PrvKeyDataCreateArgs};
// use sophis_wallet_core::storage::AccountKind;
...
async fn create_multisig(ctx: &Arc<SophisCli>, account_name: Option<String>, mnemonic_phrase_word_count: WordCount) -> Result<()> {
    let term = ctx.term();
    let wallet = ctx.wallet();
...
pub(crate) async fn bip32_watch(ctx: &Arc<SophisCli>, name: Option<&str>) -> Result<()> {
    let term = ctx.term();
    let wallet = ctx.wallet();
...
pub(crate) async fn multisig_watch(ctx: &Arc<SophisCli>, name: Option<&str>) -> Result<()> {
    let term = ctx.term();
