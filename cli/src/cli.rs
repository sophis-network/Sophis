...
use sophis_daemon::{DaemonEvent, DaemonKind, Daemons};
use sophis_wallet_core::account::Account;
use sophis_wallet_core::rpc::DynRpcApi;
use sophis_wallet_core::storage::{IdT, PrvKeyDataInfo};
use sophis_wrpc_client::{Resolver, SophisRpcClient};
...
impl From<&SophisCli> for Arc<Terminal> {
    fn from(ctx: &SophisCli) -> Arc<Terminal> {
        ctx.term()
    }
}

impl AsRef<SophisCli> for SophisCli {
    fn as_ref(&self) -> &Self {
        self
    }
}

impl workflow_log::Sink for SophisCli {
...
impl SophisCli {
    pub fn init() {
        cfg_if! {
            if #[cfg(not(target_arch = "wasm32"))] {
                ...
                sophis_core::log::set_log_level(LevelFilter::Info);
            }
        }
    ...
    pub async fn try_new_arc(options: Options) -> Result<Arc<Self>> {
        ...
        let sophis_cli = Arc::new(SophisCli {
        ...
        let term = Arc::new(Terminal::try_new_with_options(sophis_cli.clone(), options.terminal)?);
        term.init().await?;

        cfg_if! {
            if #[cfg(target_arch = "wasm32")] {
                sophis_cli.init_panic_hook();
            }
        }
    ...
    pub fn try_rpc_client(&self) -> Option<Arc<SophisRpcClient>> {
                    ...
                    msg = multiplexer.receiver.recv().fuse() => {
                        ...
                        if let Ok(msg) = msg {
                            match *msg {
                                Events::WalletList { .. } => {},
                                Events::WalletPing => {
                                    // log_info!("Sophis NG - received wallet ping");
                                },
                                Events::Metrics { network_id : _, metrics : _ } => {
                                    // log_info!("Sophis NG - received metrics event {metrics:?}")
                                }
                                Events::FeeRate { .. } => {},
                                ...
                                Events::UtxoIndexNotEnabled { .. } => {
                                    tprintln!(this, "Error: Sophis node UTXO index is not enabled...")
                                },
                                ...
                                Events::ServerStatus {
                                    ...
                                    tprintln!(this, "Connected to Sophis node version {server_version} at {}", url.unwrap_or("N/A".to_string()));

                                    let is_open = this.wallet.is_open();

                                    if !is_synced {
                                        if is_open {
                                            terrorln!(this, "Unable to update the wallet state - Sophis node is currently syncing with the network...");

                                        } else {
                                            terrorln!(this, "Sophis node is currently syncing with the network, please wait for the sync to complete...");
                                        }
                                    }
    ...
    pub async fn shutdown(&self) -> Result<()> {
        if !self.shutdown.load(Ordering::SeqCst) {
            ...
            let sophisd = self.daemons().try_sophisd();
            ...
            if let Some(sophisd) = sophisd.as_ref() {
                sophisd.mute(false).await?;
                sophisd.stop().await?;
            }
...
impl Cli for SophisCli {
    ...
    fn prompt(&self) -> Option<String> {
        ...
        if let Some(descriptor) = self.wallet.descriptor() {
            ...
            if title.to_lowercase().as_str() != "sophis" {
                prompt.push(title);
            }
...
impl cli::Context for SophisCli {
...
pub async fn sophis_cli(terminal_options: TerminalOptions, banner: Option<String>) -> Result<()> {
    SophisCli::init();

    let options = Options::new(terminal_options, None);
    let cli = SophisCli::try_new_arc(options).await?;

    let banner =
        banner.unwrap_or_else(|| format!("Sophis Cli Wallet v{} (type 'help' for list of commands)", env!("CARGO_PKG_VERSION")));
