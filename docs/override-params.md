...
The `--override-params-file` flag lets you run `sophisd` with a custom set of
...
3. Launch `sophisd` with the flag:

	 ```bash
	 sophisd --devnet --override-params-file /path/to/overrides.json
	 ```

If the file cannot be read or parsed, `sophisd` prints the error and exits.
...
If you want to run `sophisd` with the simpa generated database, you'll need to ask it to generate an override params file as well, so you can run `sophisd` with the same parameters.
...
And then launch sophisd with:
```bash
sophisd --simnet
```

and immedeiately close it. This will create the simnet datadir at `~/.rusty-sophis/sophis-simnet`.

You can then override it with the simpa database by running:
```bash
rm -rf ~/.rusty-sophis/sophis-simnet/datadir/consensus/consensus-001/
mv /path/to/simpa/database ~/.rusty-sophis/sophis-simnet/datadir/consensus/consensus-001/
```

And finally launch sophisd with:
```bash
sophisd --simnet --override-params-file overrides.json
```