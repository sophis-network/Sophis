# Running Sophis Archive Nodes

This guide explains how to run a Sophis archive node with HDD-optimized RocksDB configuration.
...
Normal Sophis nodes are **pruned** and only keep recent blocks (determined by pruning depth).
...
Sophisd provides two RocksDB configuration presets optimized for different storage types:

### Default Preset (SSD/NVMe)
```bash
sophisd --archival
# or explicitly:
sophisd --archival --rocksdb-preset=default
```
...
### HDD Preset
```bash
sophisd --archival --rocksdb-preset=hdd
```
...
sophisd --archival \
  --rpclisten-borsh=0.0.0.0:47110 \
  --rpclisten-json=0.0.0.0:48110
```
...
```bash
# /etc/sysctl.d/90-sophisd-archive.conf
vm.dirty_ratio = 40
...
Apply with: `sudo sysctl -p /etc/sysctl.d/90-sophisd-archive.conf`
...
sophisd --archival --rocksdb-preset=hdd --ram-scale=0.2

# Normal RAM (8GB system)
sophisd --archival --rocksdb-preset=hdd --ram-scale=0.5

# High RAM (16GB+ system)
sophisd --archival --rocksdb-preset=hdd --ram-scale=1.0
```
...
sophisd --archival --rocksdb-preset=hdd --rocksdb-cache-size=2048  # 2GB cache
```
...
```bash
# Using sophis-cli (if installed)
sophis-cli getinfo

# Check logs
journalctl -u sophisd -f

# Check disk usage
du -sh ~/.sophis/sophis-mainnet/datadir/
```

### Performance Metrics
```bash
# Enable performance metrics
sophisd --archival --rocksdb-preset=hdd --perf-metrics --perf-metrics-interval-sec=60
```
...
services:
  sophisd-archive:
    image: sophisnet/sophisd:latest
    container_name: sophisd-archive
    ...
    command:
      ...
      - --utxoindex
    volumes:
      - /mnt/hdd/sophis-archive:/app/data
    ports:
      - "46111:46111"  # P2P
      - "47110:47110"  # RPC Borsh
      - "48110:48110"  # RPC JSON
    environment:
      - SPHSPAD_APPDIR=/app/data
```
...
# Run container
docker run -d \
  --name sophisd-archive \
  --restart unless-stopped \
  -v /mnt/hdd/sophis-archive:/app/data \
  -p 46111:46111 \
  -p 47110:47110 \
  -p 48110:48110 \
  sophisnet/sophisd:latest \
    --archival \
    --rocksdb-preset=hdd \
    --ram-scale=1.0 \
    --rpclisten-borsh=0.0.0.0:47110 \
...
**/etc/systemd/system/sophisd-archive.service:**
```ini
[Unit]
Description=Sophis Archive Node (HDD-optimized)
...
sudo systemctl enable sophisd-archive
sudo systemctl start sophisd-archive
sudo systemctl status sophisd-archive
```
...
1. Verify HDD preset is active:
   ```bash
   journalctl -u sophisd-archive | grep "RocksDB preset"
...
2. Reduce `--ram-scale` and cache size:
   ...
   sophisd --archival --rocksdb-preset=hdd --rocksdb-cache-size=64
   ```
...
systemctl cat sophisd-archive | grep rocksdb-preset

# Check startup logs
journalctl -u sophisd-archive -n 100 | grep -i rocksdb
