...
1. **Obtain Sophis v1.0.0 binaries**  
    Download and extract the official [1.0.0 release](https://github.com/sophisnet/rusty-sophis/releases/tag/v1.0.0), or build from the `master` branch by following the instructions in the project README.
...
2. **Launch the Node**  
    ...
    *(If running from source code:)*  
    ```
    cargo run --bin sophisd --release -- --utxoindex
    ```

    To run on testnet, simply add `--testnet` at the end. For example:

    ```
    sophisd --utxoindex --testnet
    ```
...
Make sure that you get the updated `message.proto` and `rpc.proto` from the rusty-sophis repo at https://github.com/sophisnet/rusty-sophis/tree/master/rpc/grpc/core/proto

#### Using the golang Sophisd repo as an SDK

If you use the golang Sophisd repo as an SDK, a new tag [v0.12.20](https://github.com/sophisnet/sophisd/releases/tag/v0.12.20) has been made which contains the changes you would need to get your pool/stratum updated. Update your dependency on the old repo to that tag.
