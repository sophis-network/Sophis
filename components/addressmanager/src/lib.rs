...
use sophis_core::{debug, info, task::tick::TickService, time::unix_now, warn};
use sophis_database::prelude::{CachePolicy, DB, StoreResultExt};
...
pub(crate) const UPNP_REGISTRATION_NAME: &str = "rusty-sophis";
...
mod address_store_with_cache {
    ...
    impl RandomWeightedIterator {
        pub fn new(weights: Vec<f64>, addresses: Vec<NetAddress>) -> Self {
            ...
            let weighted_index = match WeightedIndex::new(weights) {
                Ok(index) => Some(index),
                Err(WeightedError::InvalidInput) => None,
                Err(e) => panic!("{e}"),
            };
        ...
        fn next(&mut self) -> Option<Self::Item> {
            if let Some(weighted_index) = self.weighted_index.as_mut() {
                ...
                match weighted_index.update_weights(&[(i, &0f64)]) {
                    Ok(_) => {}
                    Err(WeightedError::InsufficientNonZero) => self.weighted_index = None,
                    Err(e) => panic!("{e}"),
                }
                self.remaining -= 1;
                if self.remaining == 0 {
    ...
    mod tests {
        ...
        use sophis_consensus_core::config::{Config, params::SIMNET_PARAMS};
