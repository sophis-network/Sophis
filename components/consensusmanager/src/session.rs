...
impl ConsensusSessionOwned {
    ...
    pub async fn async_da_get_payload(&self, payload_id: [u8; 48]) -> Option<PayloadEntry> {
        self.clone().spawn_blocking(move |c| c.da_get_payload(payload_id)).await
    }

    pub async fn async_da_get_bundle_index(&self, bundle_id: [u8; 48]) -> Option<BundleIndex> {
        self.clone().spawn_blocking(move |c| c.da_get_bundle_index(bundle_id)).await
    }

    pub async fn async_da_list_by_block(&self, block_hash: Hash) -> Vec<[u8; 48]> {
        self.clone().spawn_blocking(move |c| c.da_list_by_block(block_hash)).await
    }

    pub async fn async_da_list_by_domain(&self, domain_byte: u8, blue_score: u64) -> Vec<[u8; 48]> {
        self.clone().spawn_blocking(move |c| c.da_list_by_domain(domain_byte, blue_score)).await
    }
