...
> **F-22 (audit P0, fix `d7c877e`; landmine DEFUSED S16).** `calc_storage_mass` is now **total**: every division uses `checked_div` (output *and* input side), so any zero divisor (`amount == 0`, no inputs, zero mean) returns `None` (= "mass incomputable → tx rejected"), **never a divide-by-zero panic**. Forgetting to add a new value==0 output type to `is_zero_value_protocol_output()` in `mass/mod.rs` is therefore **no longer a P0 chain halt** — at worst that one new output kind's txs are spuriously rejected (P2) until the allow-list is updated. The allow-list is now a *correctness* layer (don't reject recognized carriers/ALT), not the sole safety net. Still: when adding a new consensus-mandated value==0 output kind, add it to `is_zero_value_protocol_output()` **and** give it a non-contextual mass contribution (else its txs are rejected), plus a `mass::tests` unit test mirroring `test_storage_mass_skips_v5_carrier`.
