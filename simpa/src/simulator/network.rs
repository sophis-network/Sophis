...
use sophis_consensus::config::Config;
...
use sophis_database::{create_permanent_db, create_temp_db};
use sophis_utils::fd_budget;
use sophis_utils::sim::Simulation;

type ConsensusWrapper = (Arc<Consensus>, Vec<JoinHandle<()>>, DbLifetime);

pub struct SophisNetworkSimulator {
...
impl SophisNetworkSimulator {
    ...
    pub fn init(
        ...
        for i in 0..num_miners {
            ...
            // Each miner gets a unique Dilithium address derived from its index
            let mut seed = [0u8; 32];
            seed[..8].copy_from_slice(&i.to_le_bytes());
            let address = Address::new(Prefix::Simnet, Version::PubKeyDilithium, &seed);
