...
use crate::chips::ed25519::verify_air_chunked::{
    NUM_BOUNDARY_LIMBS, NUM_COLS, NUM_PUBLIC_VALUES, VerifyAirChunkedChip, build_verify_trace_chunked,
};
...
pub fn prove_verify_air_chunked(
    ...
    if !trace.values.len().is_multiple_of(NUM_COLS) {
        return Err(VerifyAirChunkedProverError::BadTraceShape { got: trace.values.len(), want: NUM_COLS });
    }
    ...
    let bytes = bincode::serialize(&proof).map_err(|e| VerifyAirChunkedProverError::Serialization(e.to_string()))?;
...
pub fn verify_verify_air_chunked_proof(
    ...
    let proof: p3_uni_stark::Proof<crate::config::OracleStarkConfig> =
        bincode::deserialize(proof_bytes).map_err(|e| VerifyAirChunkedVerifyError::Deserialization(e.to_string()))?;

    let public_values = build_public_values(public_key, signature, boundary);
    if public_values.len() != NUM_PUBLIC_VALUES {
        return Err(VerifyAirChunkedVerifyError::BadPublicValuesLen { got: public_values.len(), want: NUM_PUBLIC_VALUES });
    }
