//! `sha512::word64_and` — 64-bit bitwise AND AIR chip.
//!
//! Mirrors `word64_xor` in layout — bit-decomposition of every word, plus
//! chunk-level interface columns for composition with the rest of the
//! SHA-512 chip stack. The only difference is the per-bit constraint:
//! AND is the simplest bit operation, `c_bit = a_bit · b_bit` (degree 2).
//!
//! Together with `word64_xor` and a future `word64_not` chip, this gives
//! the SHA-512 round access to all of Ch and Maj (which are built from
//! AND, NOT, and XOR).
//!
...
//! ## Constraints
//!
//! - 192 boolean checks: every bit cell satisfies `b·(1-b) = 0`.
//! - 12 chunk-recomposition checks: chunk = Σ bit[k]·2^k.
//! - 64 per-bit AND checks: `c_bit - a_bit · b_bit = 0`.
//!
//! ## Soundness
//!
//! Sound stand-alone — same reasoning as `word64_xor`. The chunk cols
//! are bound to the bit cols by the recomposition constraints, so an
//! out-of-range chunk cannot decompose consistently. No lookup-arg
//! dependency.
...
pub const NUM_CONSTRAINTS: usize = 3 * NUM_BITS + 3 * NUM_CHUNKS + NUM_BITS; // 268
...
impl Word64AndChip {
    ...
    pub fn emit<AB: AirBuilder>(&self, builder: &mut AB) {
        ...
        for i in 0..NUM_BITS {
            ...
            builder.assert_eq(c_bit, a_bit.into() * b_bit.into());
        }
    }
}

#[derive(Debug, Clone, Copy)]
pub struct Word64AndTestAir;

impl<F: Field> BaseAir<F> for Word64AndTestAir {
    fn width(&self) -> usize {
        NUM_COLS
    }
...
impl<AB: AirBuilder> Air<AB> for Word64AndTestAir
where
    AB::F: Field,
{
    fn eval(&self, builder: &mut AB) {
        Word64AndChip::new().emit(builder);
    }
}

#[derive(Debug, Clone, Copy)]
pub struct Word64AndWitness {
...
pub fn compute_and64(a: u64, b: u64) -> Word64AndWitness {
    let c = a & b;
...
pub fn build_test_trace<F: Field + PrimeCharacteristicRing>(w: &Word64AndWitness) -> RowMajorMatrix<F> {
...
mod tests {
    ...
    #[test]
    fn and64_zero_and_zero_is_zero() {
        let w = compute_and64(0, 0);
    ...
    #[test]
    fn and64_with_zero_is_zero() {
        let w = compute_and64(0xCAFE_BABE_DEAD_BEEF, 0);
        assert_eq!(recompose_u64(&w.c_chunks), 0);
        let trace = build_test_trace::<BabyBear>(&w);
        check_constraints(&Word64AndTestAir, &trace, &[]);
    }
    ...
    #[test]
    fn and64_with_all_ones_is_identity() {
        let w = compute_and64(0xCAFE_BABE_1234_5678, u64::MAX);
    ...
    #[test]
    fn and64_self_is_identity() {
        let w = compute_and64(0xDEAD_BEEF_CAFE_BABE, 0xDEAD_BEEF_CAFE_BABE);
        assert_eq!(recompose_u64(&w.c_chunks), 0xDEAD_BEEF_CAFE_BABE);
        let trace = build_test_trace::<BabyBear>(&w);
        check_constraints(&Word64AndTestAir, &trace, &[]);
    }

    #[test]
    fn and64_complementary_masks_yield_zero() {
        // 0xAAAA... AND 0x5555... = 0 (no overlapping bits).
        let w = compute_and64(0xAAAA_AAAA_AAAA_AAAA, 0x5555_5555_5555_5555);
        assert_eq!(recompose_u64(&w.c_chunks), 0);
        let trace = build_test_trace::<BabyBear>(&w);
        check_constraints(&Word64AndTestAir, &trace, &[]);
    }
    ...
    #[test]
    fn and64_commutativity() {
        let a = 0x1234_5678_9ABC_DEF0;
        let b = 0xFEDC_BA09_8765_4321;
        let w_ab = compute_and64(a, b);
        let w_ba = compute_and64(b, a);
        assert_eq!(w_ab.c_chunks, w_ba.c_chunks);
    }

    #[test]
    fn and64_idempotence() {
        // (a AND b) AND b = a AND b
        let a = 0xCAFE_BABE_DEAD_BEEF;
        let b = 0x1234_5678_9ABC_DEF0;
        let ab = a & b;
        let w = compute_and64(ab, b);
        assert_eq!(recompose_u64(&w.c_chunks), ab);
        let trace = build_test_trace::<BabyBear>(&w);
        check_constraints(&Word64AndTestAir, &trace, &[]);
    }

    #[test]
    fn and64_matches_native_for_corner_cases() {
        let cases: [(u64, u64); 8] = [
            (0, 0),
            (1, 1),
            (1, 0),
        ...
        for (a, b) in cases {
            let w = compute_and64(a, b);
            assert_eq!(recompose_u64(&w.c_chunks), a & b, "mismatch for {a:#x} AND {b:#x}");
            let trace = build_test_trace::<BabyBear>(&w);
            check_constraints(&Word64AndTestAir, &trace, &[]);
        }
    }
    ...
    fn and64_rejects_tampered_c_bit() {
        let w = compute_and64(0xFFFF, 0xFFFF);
    ...
    #[test]
    #[should_panic(expected = "constraints not satisfied")]
    fn and64_rejects_non_boolean_b_bit() {
        let w = compute_and64(0, 0);
        let mut trace = build_test_trace::<BabyBear>(&w);
        trace.values[col::B_BITS] = BabyBear::from_u64(2);
        check_constraints(&Word64AndTestAir, &trace, &[]);
    }
