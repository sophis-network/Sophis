...
// sophis_pow used inline for PoW validation
...
impl ShareHandler {
    ...
    pub async fn handle_submit(
        ...
        event: JsonRpcEvent,
        sophis_api: Arc<dyn SophisApiTrait + Send + Sync>,
        ...
        if let Some(Value::String(submitted_identity)) = event.params.first() {
            ...
            let submitted_clean = submitted_address.trim_start_matches("sophis:").trim_start_matches("sophistest:");
            let authorized_clean = wallet_addr.trim_start_matches("sophis:").trim_start_matches("sophistest:");
        ...
        loop {
            ...
            // Use sophis_pow::State for PoW validation against the header's compact bits target.
            ...
            // IMPORTANT: Use sophis_pow's own compact-target handling as the source of truth.
            ...
            debug!("[SUBMIT]   - check_passed(sophis_pow): {}", check_passed);
            ...
            if meets_network_target {
                ...
                // The header comes directly from the Sophis node via get_block_template_call()
                // We preserve: version, bits, timestamp, all hash fields, parents, scores, etc.
                header_clone.nonce = nonce_val;

                // Verify timestamp is still valid (not too old)
                // Sophis typically accepts blocks with timestamps within a reasonable window
                ...
                debug!(
                    ...
                    "Using REAL header from Sophis node block template"
                );
                ...
                debug!("{} {}", LogColors::block("[BLOCK]"), "Calling sophis_api.submit_block()...");

                // Submit block to node
                let block_submit_result = sophis_api.submit_block(block.clone()).await;
                    ...
                    Ok(response) => {
                        ...
                        tokio::spawn(async move {
                            for _ in 0..BLOCK_CONFIRM_MAX_ATTEMPTS {
                                match sophis_api.get_current_block_color(&block_hash_for_confirm).await {
                                    Ok(true) => {
...
// Trait for sophis API operations
#[async_trait::async_trait]
pub trait SophisApiTrait: Send + Sync {
