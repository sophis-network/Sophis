...
pub fn init_metrics() {
    ...
    BLOCK_ACCEPTED_COUNTER.get_or_init(|| {
        register_counter_vec!(
            ...
            "Number of blocks accepted by the connected Sophis node (may later be red)",
            WORKER_LABELS
        )
        .unwrap()
    });
    ...
    #[cfg(feature = "rkstratum_cpu_miner")]
    {
        ...
        INTERNAL_CPU_BLOCKS_ACCEPTED_TOTAL.get_or_init(|| {
            register_counter!(
                ...
                "Total blocks accepted by the connected Sophis node from the internal CPU miner since process start"
            )
            .unwrap()
        });
...
async fn handle_http_request(
    ...
    if request.starts_with("GET /api/status") {
        let sophisd_version = crate::sophisapi::NODE_STATUS.lock().server_version.clone().unwrap_or_else(|| "-".to_string());
        ...
        let status = WebStatusResponse {
            sophisd_address: status_cfg.sophisd_address,
            sophisd_version,
...
struct WebStatusResponse {
    sophisd_address: String,
    sophisd_version: String,
    instances: usize,
    web_bind: String,
}
...
pub fn set_web_status_config(sophisd_address: String, instances: usize) {
    let lock =
        WEB_STATUS_CONFIG.get_or_init(|| parking_lot::RwLock::new(WebStatusConfig { sophisd_address: "-".to_string(), instances: 1 }));
    *lock.write() = WebStatusConfig { sophisd_address, instances: instances.max(1) };
}

fn get_web_status_config() -> WebStatusConfig {
    WEB_STATUS_CONFIG
        .get_or_init(|| parking_lot::RwLock::new(WebStatusConfig { sophisd_address: "-".to_string(), instances: 1 }))
        .read()
        .clone()
}
...
pub fn record_balances(instance_id: &str, balances: &[(String, u64)]) {
    if let Some(gauge) = BALANCE_GAUGE.get() {
        for (address, balance) in balances {
            // Convert from sompi to SPHS (divide by 100000000)
            let balance_sphs = *balance as f64 / 100_000_000.0;
            gauge.with_label_values(&[instance_id, address]).set(balance_sphs);
        }
    }
}
...
async fn get_config_json() -> String {
        ...
        && let Ok(config) = BridgeConfig::from_yaml(&content)
    {
        ...
        let json_value = serde_json::json!({
            // Global fields
            "sophisd_address": config.global.sophisd_address,
...
async fn update_config_from_json(json_body: &str) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    if let Some(addr) = updates.get("sophisd_address").and_then(|v| v.as_str()) {
        config.global.sophisd_address = addr.to_string();
    }
...
"#,
        ...
        set_web_status_config("127.0.0.1:46110".to_string(), 2);
        ...
        assert!(status_resp.contains("\"sophisd_address\""));
        ...
        assert!(config_resp.contains("\"sophisd_address\""));
        ...
        let json_body = r#"{"sophisd_address":"127.0.0.2:46110","stratum_port":":5556","min_share_diff":4096}"#;
