use crate::{
    ...
    share_handler::{ShareHandler, SophisApiTrait},
    sophisapi::SophisApi,
...
/// Start block template listener with concrete SophisApi
...
pub async fn start_block_template_listener_with_api(
    ...
    let sophis_api_cb = Arc::clone(&sophis_api);

    let block_cb = move || {
        let client_handler = Arc::clone(&client_handler_cb);
        let sophis_api = Arc::clone(&sophis_api_cb);
        tokio::spawn(async move {
            client_handler.new_block_available(sophis_api).await;
        });
    };
...
pub async fn listen_and_serve<T: SophisApiTrait + Send + Sync + 'static>(
    config: BridgeConfig,
    sophis_api: Arc<T>,
    // Optional: if concrete SophisApi is provided, use notification-based listener
    concrete_sophis_api: Option<Arc<SophisApi>>,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    listen_and_serve_impl(config, sophis_api, concrete_sophis_api, None).await
}

pub async fn listen_and_serve_with_shutdown<T: SophisApiTrait + Send + Sync + 'static>(
    config: BridgeConfig,
    sophis_api: Arc<T>,
    concrete_sophis_api: Option<Arc<SophisApi>>,
    shutdown_rx: watch::Receiver<bool>,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    listen_and_serve_impl(config, sophis_api, concrete_sophis_api, Some(shutdown_rx)).await
}

async fn listen_and_serve_impl<T: SophisApiTrait + Send + Sync + 'static>(
    config: BridgeConfig,
    sophis_api: Arc<T>,
    concrete_sophis_api: Option<Arc<SophisApi>>,
    ...
    let authorize_handler = {
        let client_handler = Arc::clone(&client_handler);
        let sophis_api = Arc::clone(&sophis_api);
        Arc::new(move |ctx: Arc<StratumContext>, event: JsonRpcEvent| {
            let client_handler = Arc::clone(&client_handler);
            let sophis_api = Arc::clone(&sophis_api);
            let ctx_clone = Arc::clone(&ctx);
            let event_clone = event.clone();
            Box::pin(async move {
                handle_authorize(ctx_clone, event_clone, Some(client_handler), Some(sophis_api))
                    .await
                    .map_err(|e| Box::new(std::io::Error::other(e.to_string())) as Box<dyn std::error::Error + Send + Sync>)
            })
                as std::pin::Pin<Box<dyn std::future::Future<Output = Result<(), Box<dyn std::error::Error + Send + Sync>>> + Send>>
    ...
    let submit_handler = {
        ...
        Arc::new(move |ctx: Arc<StratumContext>, event: JsonRpcEvent| {
            ...
            Box::pin(async move {
                share_handler
                    .handle_submit(ctx_clone, event, sophis_api)
                    .await
                    .map_err(|e| Box::new(std::io::Error::other(e.to_string())) as Box<dyn std::error::Error + Send + Sync>)
            })
                as std::pin::Pin<Box<dyn std::future::Future<Output = Result<(), Box<dyn std::error::Error + Send + Sync>>> + Send>>
    ...
    // If concrete SophisApi is provided, use notification-based listener
    // Otherwise, use polling only (fallback for trait objects)
    if let Some(concrete_api) = concrete_sophis_api {
        // We have concrete SophisApi - use notification-based listener
        let client_handler_cb = Arc::clone(&client_handler);
        let sophis_api_cb = Arc::clone(&sophis_api);
        ...
        // Call the method directly on Arc<SophisApi> (it's an instance method taking Arc<Self>)
        ...
        // No concrete SophisApi provided - use polling only
        warn!("Using polling-based block template listener (concrete SophisApi not provided, notifications not available)");

        let client_handler_poll = Arc::clone(&client_handler);
        let sophis_api_poll = Arc::clone(&sophis_api);
            ...
            loop {
                if let Some(ref mut rx) = shutdown_rx_poll {
                    tokio::select! {
                        ...
                        _ = interval.tick() => {
                            client_handler_poll.new_block_available(Arc::clone(&sophis_api_poll)).await;
                        }
                    }
                } else {
                    interval.tick().await;
