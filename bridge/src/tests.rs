...
#[cfg(test)]
use sophis_stratum_bridge::BridgeConfig;
...
fn test_config_single_instance_mode() {
    ...
    let yaml = r#"
sophisd_address: "127.0.0.1:46110"
stratum_port: ":5555"
min_share_diff: 8192
print_stats: true
"#;
    ...
    assert_eq!(config.global.sophisd_address, "127.0.0.1:46110", "Sophisd address should be stored in global config");
}
...
fn test_normalize_port_with_colon() {
    ...
    use sophis_stratum_bridge::net_utils::normalize_port;
    ...
    assert_eq!(normalize_port(":46110"), ":46110", "Port with colon should remain unchanged");
}
...
fn test_normalize_port_without_colon() {
    ...
    assert_eq!(normalize_port("46110"), ":46110", "Port without colon should get colon prefix");
}
...
fn test_bind_addr_from_port_with_colon() {
    use sophis_stratum_bridge::net_utils::bind_addr_from_port;
    assert_eq!(bind_addr_from_port(":3030"), "0.0.0.0:3030");
    assert_eq!(bind_addr_from_port(":5555"), "0.0.0.0:5555");
    assert_eq!(bind_addr_from_port(":46110"), "0.0.0.0:46110");
}
...
fn test_stratum_method_from_str() {
    use sophis_stratum_bridge::jsonrpc_event::StratumMethod;
...
fn test_jsonrpc_event_new() {
    use serde_json::json;
    use sophis_stratum_bridge::jsonrpc_event::JsonRpcEvent;
...
fn test_jsonrpc_event_method_enum() {
    use sophis_stratum_bridge::jsonrpc_event::{JsonRpcEvent, StratumMethod};
...
fn test_jsonrpc_response_new() {
    use serde_json::json;
    use sophis_stratum_bridge::jsonrpc_event::{JsonRpcEvent, JsonRpcResponse};
...
fn test_jsonrpc_response_success() {
    ...
    use sophis_stratum_bridge::jsonrpc_event::JsonRpcResponse;
...
fn test_unmarshal_event_basic() {
    use sophis_stratum_bridge::jsonrpc_event::unmarshal_event;
...
fn test_unmarshal_response_success() {
    use sophis_stratum_bridge::jsonrpc_event::unmarshal_response;
...
fn test_error_short_code_display() {
    use sophis_stratum_bridge::errors::ErrorShortCode;
...
fn test_mining_state_initial_values() {
    ...
    // - No stored job IDs
    use sophis_stratum_bridge::mining_state::MiningState;
...
fn test_mining_state_job_management() {
    ...
    use sophis_stratum_bridge::mining_state::{Job, MiningState};
...
// These tests run with: cargo test -p sophis-stratum-bridge --bin stratum-bridge
// Or with CPU miner: cargo test -p sophis-stratum-bridge --features rkstratum_cpu_miner --bin stratum-bridge

#[cfg(test)]
mod integration {
    use sophis_alloc::init_allocator_with_default_settings;
    use sophis_stratum_bridge::{SophisApi, StratumServerBridgeConfig as StratumBridgeConfig, listen_and_serve_with_shutdown};
    use sophisd_lib::args as sophisd_args;
    ...
    async fn test_bridge_startup_with_inprocess_node() {
        ...
        let temp_dir = std::env::temp_dir().join(format!("sophisd_test_{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&temp_dir).unwrap();

        // Start an in-process node with simnet/testnet settings
        // Use a random port to avoid conflicts
        let rpc_port = 46110 + (std::process::id() % 1000) as u16;
        ...
        let bridge_handle =
            tokio::spawn(
                async move { listen_and_serve_with_shutdown::<SophisApi>(bridge_config, sophis_api, None, shutdown_rx).await },
            );
    ...
    async fn test_bridge_startup_with_cpu_miner_feature() {
        ...
        use sophis_stratum_bridge::InternalCpuMinerConfig;
        let miner_config = InternalCpuMinerConfig {
            enabled: false, // Don't actually mine in test
            mining_address: "sophistest:test".to_string(),
            threads: 1,
            throttle: None,
...
mod comprehensive_tests {
    ...
    use sophis_consensus_core::tx::{ScriptPublicKey, Transaction, TransactionOutput};
    use sophis_hashes::Hash;
    use sophis_stratum_bridge::{
    ...
    /// This helps us test share validation without needing a real Sophis node
    ...
    // - SophisDiff: Stores difficulty, target, and hash value
    // ========================================================================
    ...
    fn test_difficulty_to_target_calculation() {
        ...
        use sophis_stratum_bridge::hasher::diff_to_target;
    ...
    #[test]
    fn test_sophis_diff_structure() {
        // Test: SophisDiff structure and methods
        // This demonstrates how difficulty is stored and managed

        let mut diff = SophisDiff::new();
    ...
    fn test_difficulty_for_different_miners() {
        ...
        let mut diff1 = SophisDiff::new();
        let mut diff2 = SophisDiff::new();
        let mut diff3 = SophisDiff::new();
    ...
    fn test_mining_state_difficulty_storage() {
        ...
        let mut stratum_diff = SophisDiff::new();
    ...
    fn test_share_handler_worker_stats() {
        ...
        *ctx.wallet_addr.lock() = "sophistest:test".to_string();
    ...
    // Run with: cargo test -p sophis-stratum-bridge --features rkstratum_cpu_miner
    // ========================================================================
    ...
    fn test_internal_cpu_miner_config_creation() {
        ...
        let config = InternalCpuMinerConfig {
            enabled: true,
            mining_address: "sophistest:test123456789012345678901234567890123456789012345678901234567890".to_string(),
            threads: 4,
        ...
        assert_eq!(config.mining_address, "sophistest:test123456789012345678901234567890123456789012345678901234567890");
    ...
    fn test_internal_miner_metrics_initialization() {
        ...
        use sophis_stratum_bridge::rkstratum_cpu_miner::InternalMinerMetrics;
    ...
    async fn test_internal_cpu_miner_spawn_disabled() {
        ...
        // Create a mock SophisApi (we won't actually use it when disabled)
        ...
        // without needing a real SophisApi
    ...
    async fn test_internal_cpu_miner_integration_with_mock_api() {
        ...
        // Note: Full integration requires a real Sophis node (tested in integration tests)
        ...
        // let metrics = spawn_internal_cpu_miner(sophis_api, config, shutdown_rx).await?;
    ...
    fn test_internal_cpu_miner_pow_checking_concept() {
        ...
        use sophis_consensus_core::header::Header;
        use sophis_hashes::Hash;
        use sophis_pow::State as PowState;
    ...
    async fn test_wallet_address_cleaning_with_prefix() {
        // Test: Wallet addresses with sophis:/sophistest:/sophisdev: prefixes
        ...
        let event1 = JsonRpcEvent::new(
            Some("1".to_string()),
            "mining.authorize",
            vec![json!("sophis:qr8example123456789012345678901234567890123456789012345678901234567890")],
        );
        ...
        assert!(addr1.starts_with("sophis:"), "Address should have sophis: prefix");
        ...
        // Test sophistest: prefix
        let ctx2 = create_test_context().await;
        let event2 = JsonRpcEvent::new(
            Some("2".to_string()),
            "mining.authorize",
            vec![json!("sophistest:qr8example123456789012345678901234567890123456789012345678901234567890")],
        );
        assert_eq!(event2.params.len(), 1);
        let addr2 = event2.params[0].as_str().unwrap();
        assert!(addr2.starts_with("sophistest:"), "Address should have sophistest: prefix");
        let _result2: Result<(), _> = handle_authorize(ctx2.clone(), event2, None, None).await;

        // Test sophisdev: prefix
        let ctx3 = create_test_context().await;
        let event3 = JsonRpcEvent::new(
            Some("3".to_string()),
            "mining.authorize",
            vec![json!("sophisdev:qr8example123456789012345678901234567890123456789012345678901234567890")],
        );
        assert_eq!(event3.params.len(), 1);
        let addr3 = event3.params[0].as_str().unwrap();
        assert!(addr3.starts_with("sophisdev:"), "Address should have sophisdev: prefix");
    ...
    async fn test_wallet_address_cleaning_without_prefix() {
        // Test: Wallet addresses without prefix should get sophis: prefix added
        let ctx = create_test_context().await;
        ...
        assert!(!addr_param.starts_with("sophis:"), "Test address should not have prefix initially");

        // handle_authorize will call clean_wallet which should add sophis: prefix
    ...
    fn test_share_submit_ethereumstratum_format_validation() {
        ...
        // Note: Full submission testing requires a SophisApi mock, so we test the format here
        ...
        let event = JsonRpcEvent::new(
            ...
            "mining.submit",
            vec![
                json!("sophistest:qr8example123456789012345678901234567890123456789012345678901234567890.worker1"),
                json!("1"),
    ...
    fn test_stratum_context_summary() {
        ...
        *ctx.wallet_addr.lock() = "sophistest:qr8example123456789012345678901234567890123456789012345678901234567890".to_string();
        *ctx.worker_name.lock() = "worker1".to_string();
        *ctx.remote_app.lock() = "BzMiner".to_string();
        ...
        assert_eq!(
            summary.wallet_addr, "sophistest:qr8example123456789012345678901234567890123456789012345678901234567890",
