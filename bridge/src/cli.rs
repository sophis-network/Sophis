...
use sophis_stratum_bridge::{BridgeConfig, InstanceConfig, net_utils::normalize_port};
...
pub struct Cli {
    ...
    #[arg(last = true, help = "Sophisd arguments (use '--' separator if sophisd args start with hyphens)")]
    pub sophisd_args: Vec<String>,

    #[arg(long)]
    pub sophisd_address: Option<String>,
...
pub fn apply_cli_overrides(config: &mut BridgeConfig, cli: &Cli) -> Result<(), anyhow::Error> {
    if let Some(addr) = cli.sophisd_address.as_deref() {
