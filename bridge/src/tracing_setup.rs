    ...
    N: for<'a> FormatFields<'a> + 'static,
{
    fn format_event(
        ...
        // Special-case forwarded node logs (from `tracing_log::LogTracer`) to match sophisd style:
        ...
        if self.apply_colors {
            ...
            if message.contains("[ASIC->BRIDGE]") {
                ...
                // Configuration lines - color the label part (e.g., "\tsophisd:          value")
...
pub(crate) fn init_tracing(
    ...
    // The embedded sophisd runtime initializes the global `log` logger via
    // `sophis_core::log::init_logger(...).unwrap()`. If LogTracer grabs the
    // global logger first, sophisd panics with SetLoggerError.
