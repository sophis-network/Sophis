    ...
    ($name:ident, $n_words:literal $(, $derive_trait:ty)*) => {
        ...
        impl $name {
            ...
            pub fn mod_inverse(self, prime: Self) -> Option<Self> {
                ...
                // malachite-nz's `Limb` is u64 by default, u32 with the
                // `32_bit_limbs` feature (enabled transitively by
                // `risc0-circuit-rv32im`). We store limbs as `[u64; LIMBS]`,
                // so reinterpret the bytes at the boundary. Safe on
                // little-endian hosts: `[u64; N]` and `[u32; 2N]` share
                // identical byte layouts and `[u64; N]` is at least 4-byte
                // aligned, so the cast respects `Limb`'s alignment too.
                #[cfg(not(target_endian = "little"))]
                compile_error!("sophis-math requires little-endian targets");
                const _: () = assert!(
                    core::mem::size_of::<Limb>() == 4 || core::mem::size_of::<Limb>() == 8
                );

                let limbs_per_u64 = core::mem::size_of::<u64>() / core::mem::size_of::<Limb>();
                let self_limbs: &[Limb] = unsafe {
                    core::slice::from_raw_parts(
                        self.0.as_ptr() as *const Limb,
                        self.0.len() * limbs_per_u64,
                    )
                };
                let prime_limbs: &[Limb] = unsafe {
                    core::slice::from_raw_parts(
                        prime.0.as_ptr() as *const Limb,
                        prime.0.len() * limbs_per_u64,
                    )
                };

                let x = Natural::from_limbs_asc(self_limbs);
                let p = Natural::from_limbs_asc(prime_limbs);
                ...
                mod_inv.map(|n| {
                    ...
                    let src_bytes = unsafe {
                        core::slice::from_raw_parts(
                            ...
                            limbs.len() * core::mem::size_of::<Limb>(),
                        )
                    };
                    let dst_bytes = unsafe {
                        core::slice::from_raw_parts_mut(
                            res.as_mut_ptr() as *mut u8,
                            res.len() * core::mem::size_of::<u64>(),
                        )
                    };
        ...
        impl sophis_utils::mem_size::MemSizeEstimator for $name {
        ...
        impl sophis_utils::hex::ToHex for $name {
        ...
        impl sophis_utils::hex::FromHex for $name {
