...
use sophis_notify::address::tracker::Tracker;
use sophis_utils::networking::ContextualNetAddress;
use sophis_wrpc_server::address::WrpcNetAddress;
...
#[cfg(feature = "devnet-prealloc")]
use sophis_consensus_core::tx::{TransactionOutpoint, UtxoEntry};
#[cfg(feature = "devnet-prealloc")]
...
impl Args {
    ...
    #[cfg(feature = "devnet-prealloc")]
    pub fn generate_prealloc_utxos(&self, num_prealloc_utxos: u64) -> sophis_consensus_core::utxo::utxo_collection::UtxoCollection {
...
pub fn cli() -> Command {
    ...
    let cmd = Command::new("sophisd")
        .about(format!("{} (rusty-sophis) v{}", env!("CARGO_PKG_DESCRIPTION"), version()))
        .version(env!("CARGO_PKG_VERSION"))
        .arg(arg!(-C --configfile <CONFIG_FILE> "Path of config file.").env("SOPHISD_CONFIGFILE"))
        .arg(arg!(-b --appdir <DATA_DIR> "Directory to store data.").env("SOPHISD_APPDIR"))
        .arg(arg!(--logdir <LOG_DIR> "Directory to log output.").env("SOPHISD_LOGDIR"))
        .arg(arg!(--nologfiles "Disable logging to files.").env("SOPHISD_NOLOGFILES"))
        .arg(
            Arg::new("async_threads")
                .short('t')
                .long("async-threads")
                .env("SOPHISD_ASYNC_THREADS")
        ...
        .arg(
            Arg::new("rpclisten")
                .long("rpclisten")
                .env("SOPHISD_RPCLISTEN")
                .value_name("IP[:PORT]")
                .num_args(0..=1)
                .require_equals(true)
                .value_parser(clap::value_parser!(ContextualNetAddress))
                .help("Interface:port to listen for gRPC connections (default port: 46110, testnet: 46210)."),
        )
        .arg(
            Arg::new("rpclisten-borsh")
                .long("rpclisten-borsh")
                .env("SOPHISD_RPCLISTEN_BORSH")
                .value_name("IP[:PORT]")
                .num_args(0..=1)
                .require_equals(true)
                ...
                .help("Interface:port to listen for wRPC Borsh connections (default port: 47110, testnet: 47210)."),

        )
        .arg(
            Arg::new("rpclisten-json")
                .long("rpclisten-json")
                .env("SOPHISD_RPCLISTEN_JSON")
                .value_name("IP[:PORT]")
                .num_args(0..=1)
                .require_equals(true)
                .default_missing_value("default") // TODO: Find a way to use defaults.rpclisten_json
                .value_parser(clap::value_parser!(WrpcNetAddress))
                .help("Interface:port to listen for wRPC JSON connections (default port: 48110, testnet: 48210)."),
        )
        .arg(arg!(--unsaferpc "Enable RPC commands which affect the state of the node").env("SOPHISD_UNSAFERPC"))
        .arg(
            Arg::new("connect-peers")
                .long("connect")
                .env("SOPHISD_CONNECTPEERS")
                .value_name("IP[:PORT]")
        ...
        .arg(
            Arg::new("listen")
                ...
                .require_equals(true)
                .value_parser(clap::value_parser!(ContextualNetAddress))
                .help("Add an interface:port to listen for connections (default all interfaces port: 46111, testnet: 46211)."),
        )
        .arg(
            Arg::new("outpeers")
                .long("outpeers")
                .env("SOPHISD_OUTPEERS")
                .value_name("outpeers")
                .require_equals(true)
                .value_parser(clap::value_parser!(usize))
        ...
        .arg(
            Arg::new("rpcmaxclients")
                .long("rpcmaxclients")
                .env("SOPHISD_RPCMAXCLIENTS")
                .value_name("rpcmaxclients")
                .require_equals(true)
                .value_parser(clap::value_parser!(usize))
                .help("Max number of RPC clients for standard connections (default: 128)."),
        )
        .arg(arg!(--"reset-db" "Reset database before starting node. It's needed when switching between subnetworks.").env("SOPHISD_RESET_DB"))
        .arg(arg!(--"enable-unsynced-mining" "Allow the node to accept blocks from RPC while not synced (this flag is mainly used for testing)").env("SOPHISD_ENABLE_UNSYNCED_MINING"))
        .arg(
            Arg::new("enable-mainnet-mining")
                .long("enable-mainnet-mining")
                .env("SOPHISD_ENABLE_MAINNET_MINING")
                .action(ArgAction::SetTrue)
                .hide(true)
                .help("Allow mainnet mining (currently enabled by default while the flag is kept for backwards compatibility)"),
        )
        .arg(arg!(--utxoindex "Enable the UTXO index").env("SOPHISD_UTXOINDEX"))
        .arg(
            Arg::new("max-tracked-addresses")
                .long("max-tracked-addresses")
                .env("SOPHISD_MAX_TRACKED_ADDRESSES")
                .require_equals(true)
                .value_parser(clap::value_parser!(usize))
...
0, Tracker::MAX_ADDRESS_UPPER_BOUND, Tracker::DEFAULT_MAX_ADDRESSES)),
        ...
        .arg(arg!(--testnet "Use the test network").env("SOPHISD_TESTNET"))
        .arg(
            Arg::new("netsuffix")
                .long("netsuffix")
                .env("SOPHISD_NETSUFFIX")
                .value_name("netsuffix")
                .require_equals(true)
                .value_parser(clap::value_parser!(u32))
                .help("Testnet network suffix number"),
        )
        .arg(arg!(--devnet "Use the development test network").env("SOPHISD_DEVNET"))
        .arg(arg!(--simnet "Use the simulation test network").env("SOPHISD_SIMNET"))
        .arg(arg!(--archival "Run as an archival node: avoids deleting old block data when moving the pruning point (Warning: heavy disk usage)").env("SOPHISD_ARCHIVAL"))
        .arg(arg!(--sanity "Enable various sanity checks which might be compute-intensive (mostly performed during pruning)").env("SOPHISD_SANITY"))
        .arg(arg!(--yes "Answer yes to all interactive console questions").env("SOPHISD_NONINTERACTIVE"))
        .arg(
            Arg::new("user_agent_comments")
                .long("uacomment")
                .env("SOPHISD_USER_AGENT_COMMENTS")
                .action(ArgAction::Append)
                .require_equals(true)
        ...
        .arg(arg!(--"perf-metrics" "Enable performance metrics: cpu, memory, disk io usage").env("SOPHISD_PERF_METRICS"))
        .arg(
            Arg::new("perf-metrics-interval-sec")
                .long("perf-metrics-interval-sec")
                .env("SOPHISD_PERF_METRICS_INTERVAL_SEC")
                .require_equals(true)
                .value_parser(clap::value_parser!(u64))
                .help("Interval in seconds for performance metrics collection."),
        )
        .arg(arg!(--"disable-upnp" "Disable upnp").env("SOPHISD_DISABLE_UPNP"))
        .arg(arg!(--"nodnsseed" "Disable DNS seeding for peers").env("SOPHISD_NODNSSEED"))
        .arg(arg!(--"nogrpc" "Disable gRPC server").env("SOPHISD_NOGRPC"))
        .arg(
            Arg::new("ram-scale")
                .long("ram-scale")
                .env("SOPHISD_RAM_SCALE")
                .require_equals(true)
...
/*
  ...
  -a, --addpeer=                            Add a peer to connect with at startup
      ...
      --whitelist=                          Add an IP network or IP that will not be banned. (eg. 192.168.1.0/24 or
                                            ::1)
      --rpclisten=                          Add an interface/port to listen for RPC connections (default port: 46110,
                                            testnet: 46210)
      --rpccert=                            File containing the certificate file (default:
                                            /Users/aspect/Library/Application Support/Sophisd/rpc.cert)
      --rpckey=                             File containing the certificate key (default:
                                            /Users/aspect/Library/Application Support/Sophisd/rpc.key)
  ...
  -d, --loglevel=                           Logging level for all subsystems {trace, debug, info, warn, error,
      ...
      --minrelaytxfee=                      The minimum transaction fee in SPHS/kB to be considered a non-zero fee.
                                            (default: 1e-05)
