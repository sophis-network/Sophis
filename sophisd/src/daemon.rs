...
use sophis_consensus_notify::{root::ConsensusNotificationRoot, service::NotifyService};
use sophis_core::{core::Core, debug, info};
use sophis_core::{sophisd_env::version, task::tick::TickService};
use sophis_database::{
    prelude::{CachePolicy, DbWriter, DirectDbWriter, RocksDbPreset},
    registry::DatabaseStorePrefixes,
};
use sophis_grpc_server::service::GrpcService;
use sophis_notify::{address::tracker::Tracker, subscription::context::SubscriptionContext};
use sophis_p2p_lib::Hub;
use sophis_p2p_mining::rule_engine::MiningRuleEngine;
use sophis_rpc_service::service::RpcCoreService;
use sophis_txscript::caches::TxScriptCacheCounters;
use sophis_utils::git;
...
use sophis_p2p_flows::{flow_context::FlowContext, service::P2pService};

use sophis_perf_monitor::{builder::Builder as PerfMonitorBuilder, counters::CountersSnapshot};
use sophis_utxoindex::{UtxoIndex, api::UtxoIndexProxy};
use sophis_wrpc_server::service::{Options as WrpcServerOptions, WebSocketCounters as WrpcServerCounters, WrpcEncoding, WrpcService};

/// Desired soft FD limit that needs to be configured
/// for the sophisd process.
pub const DESIRED_DAEMON_SOFT_FD_LIMIT: u64 = 8 * 1024;
/// Minimum acceptable soft FD limit for the sophisd
/// process. (Rusty Sophis will operate with the minimal
...
pub fn get_app_dir() -> PathBuf {
    #[cfg(target_os = "windows")]
    return get_home_dir().join("rusty-sophis");
    #[cfg(not(target_os = "windows"))]
    return get_home_dir().join(".rusty-sophis");
}
...
fn request_database_deletion_approval(approve: bool) -> bool {
    let msg = "Node database is from a different Sophisd *DB* version and needs to be fully deleted, do you confirm the delete? (y/n)";
    get_user_approval_or_exit(msg, approve);
    info!("Deleting databases from previous Sophisd version");
...
/// the application folder that contains sophisd logs and the database.
    ...
    pub fn from_args(args: &Args) -> Self {
        ...
        cfg_if::cfg_if! {
            if #[cfg(feature = "semaphore-trace")] {
                sophis_core::log::init_logger(log_dir.as_deref(), &format!("{},{}=debug", args.log_level, sophis_utils::sync::semaphore_module_path()));
            } else {
                sophis_core::log::init_logger(log_dir.as_deref(), &args.log_level);
            }
        };
    ...
    let params = {
        ...
        match &args.override_params_file {
            Some(path) => {
                ...
                // OverrideParams may carry borrowed string fields, so serde_json
                // requires the input to be 'static. We leak the one-time startup
                // string to satisfy this constraint.
                let file_content: &'static str = Box::leak(
...
do you confirm? (answer y/n or pass --yes to the Sophisd command line to confirm all interactive questions)";
    ...
    let mut meta_db = sophis_database::prelude::ConnBuilder::default()
    ...
    if !is_db_reset_needed && (args.testnet || args.devnet || args.simnet) {
        ...
        match active_consensus_dir_name {
            Some(dir_name) => {
                let consensus_db = sophis_database::prelude::ConnBuilder::default()
    ...
    // Reset Condition: Need to reset if we're upgrading from sophisd DB version
...
Do you confirm? (y/n)";
    ...
    if is_db_reset_needed && !args.reset_db {
        ...
        // Reopen the DB
        meta_db = sophis_database::prelude::ConnBuilder::default()
    ...
    let perf_monitor = if args.perf_metrics {
        let cb = move |counters: CountersSnapshot| {
            debug!("[{}] {}", sophis_perf_monitor::SERVICE_NAME, counters.to_process_metrics_display());
            debug!("[{}] {}", sophis_perf_monitor::SERVICE_NAME, counters.to_io_metrics_display());
            #[cfg(feature = "heap")]
            debug!("[{}] heap stats: {:?}", sophis_perf_monitor::SERVICE_NAME, dhat::HeapStats::get());
        };
    ...
    let index_service: Option<Arc<IndexService>> = if args.utxoindex {
        ...
        let utxoindex_db = sophis_database::prelude::ConnBuilder::default()
