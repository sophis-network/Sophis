extern crate sophis_consensus;
extern crate sophis_core;
extern crate sophis_hashes;

use std::sync::Arc;

use sophis_alloc::init_allocator_with_default_settings;
use sophis_core::{info, signals::Signals};
...
pub fn main() {
    #[cfg(feature = "heap")]
    let _profiler = dhat::Profiler::builder().file_name("sophisd-heap.json").build();
    ...
    match fd_budget::try_set_fd_limit(DESIRED_DAEMON_SOFT_FD_LIMIT) {
        Ok(limit) => {
            if limit < MINIMUM_DAEMON_SOFT_FD_LIMIT {
                ...
                println!("The sophisd node requires a setting of at least {DESIRED_DAEMON_SOFT_FD_LIMIT} to operate properly.");
