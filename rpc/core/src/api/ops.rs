...
pub enum RpcApiOps {
    ...
    GetSyncStatus = 115,
    /// Returns the network this Sophisd is connected to (Mainnet, Testnet)
    ...
    /// Returns a list of all the addresses (IP, port) this Sophisd knows and a list of all addresses that are currently banned by this Sophisd
    ...
    /// Returns a list of the peers currently connected to this Sophisd, along with some statistics on them
    GetConnectedPeerInfo = 123,
    /// Instructs Sophisd to connect to a given IP address.
    AddPeer = 124,
    ...
    /// Get a single V5 carrier payload by its 48-byte SHA3-384 payload_id.
    GetDaPayload = 152,
    /// Get a reassembled DA bundle (all fragments concatenated + verified) by bundle_id.
    GetDaBundle = 153,
    /// List the payload_ids accepted by a specific block hash.
    GetDaCarriersByBlock = 154,
    /// List the payload_ids in a (domain_byte, blue_score_bucket) cell.
