//!
//! The main [`RpcApi`] trait that defines all RPC methods available in the Rusty Sophis p2p node.
//!
...
use sophis_notify::{listener::ListenerId, scope::Scope, subscription::Command};
use std::sync::Arc;
...
pub trait RpcApi: Sync + Send + AnySync {
    ...
    /// Requests the list of known sophisd addresses in the current network (mainnet, testnet, etc.)
    ...
    // See `oracle/docs/PHASE6_DA_DESIGN.md` §6.2.
    // ----------------------------------------------------------------

    /// Returns one V5 carrier fragment by its 48-byte SHA3-384 payload_id.
    async fn get_da_payload(&self, payload_id: Vec<u8>) -> RpcResult<Option<RpcDaPayload>> {
        Ok(self.get_da_payload_call(None, GetDaPayloadRequest::new(payload_id)).await?.entry)
    }
    ...
    /// Returns a reassembled DA bundle (all fragments concatenated and
    /// verified against `bundle_id`) by 48-byte SHA3-384 bundle_id.
    /// `bundle.data == None` means at least one fragment is missing.
    async fn get_da_bundle(&self, bundle_id: Vec<u8>) -> RpcResult<Option<RpcDaBundle>> {
        Ok(self.get_da_bundle_call(None, GetDaBundleRequest::new(bundle_id)).await?.bundle)
    }
    ...
    /// Lists payload_ids accepted by `block_hash`. Empty `Vec` if the block
    /// has no carriers; an error path returns it via the underlying call.
    async fn get_da_carriers_by_block(&self, block_hash: RpcHash) -> RpcResult<Vec<Vec<u8>>> {
        Ok(self.get_da_carriers_by_block_call(None, GetDaCarriersByBlockRequest::new(block_hash)).await?.payload_ids)
    }
    ...
    /// Returns the confirmation status of a payload_id.
    /// `None` means the payload is unknown (either pruned or never accepted).
    async fn get_da_payload_status(&self, payload_id: Vec<u8>) -> RpcResult<Option<RpcDaPayloadStatus>> {
        Ok(self.get_da_payload_status_call(None, GetDaPayloadStatusRequest::new(payload_id)).await?.status)
    }
