...
use sophis_consensus_core::api::stats::BlockCount;
use sophis_core::debug;
use sophis_notify::subscription::{Command, context::SubscriptionContext, single::UtxosChangedSubscription};
...
pub struct GetBlockTemplateRequest {
    /// Which sophis address should the coinbase block reward transaction pay into
...
pub struct GetBlockTemplateResponse {
    ...
    /// Whether sophisd thinks that it's synced.
    /// Callers are discouraged (but not forbidden) from solving blocks when sophisd is not synced.
    /// That is because when sophisd isn't in sync with the rest of the network there's a high
...
pub struct ConsensusMetrics {
    ...
    // Orphan rate monitoring: cumulative red and blue block counts across all chain blocks
    pub network_orphan_reds_count: u64,
    pub network_orphan_blues_count: u64,
}
...
impl Serializer for ConsensusMetrics {
    fn serialize<W: std::io::Write>(&self, writer: &mut W) -> std::io::Result<()> {
        ...
        store!(u64, &self.network_orphan_reds_count, writer)?;
        store!(u64, &self.network_orphan_blues_count, writer)?;
...
impl Deserializer for ConsensusMetrics {
    fn deserialize<R: std::io::Read>(reader: &mut R) -> std::io::Result<Self> {
        ...
        let network_orphan_reds_count = load!(u64, reader)?;
        let network_orphan_blues_count = load!(u64, reader)?;
        ...
        Ok(Self {
            ...
            network_orphan_reds_count,
            network_orphan_blues_count,
        })
    }
}
...
//
// This call is only available when this sophisd was started with `--utxoindex`
//
