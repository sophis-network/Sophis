...
use sophis_utils::{hex::ToHex, serde_bytes_fixed_ref};
...
/// Represents the ID of a Sophis transaction
...
impl From<sophis_consensus_client::TransactionOutpoint> for RpcTransactionOutpoint {
    fn from(outpoint: sophis_consensus_client::TransactionOutpoint) -> Self {
        TransactionOutpoint::from(outpoint).into()
    }
}

impl From<RpcTransactionOutpoint> for sophis_consensus_client::TransactionOutpoint {
    fn from(outpoint: RpcTransactionOutpoint) -> Self {
        TransactionOutpoint::from(outpoint).into()
    }
}
...
/// Represent Sophis transaction input verbose data
...
/// Represent Sophis transaction output verbose data
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Represent Sophis transaction verbose data
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
