...
use sophis_wrpc_server::{
    connection::Connection,
    router::Router,
    server::Server,
    service::{Options, SophisRpcHandler},
};
use std::sync::Arc;
use workflow_log::*;
...
struct Args {
    ...
    /// proxy:port for gRPC server (grpc://127.0.0.1:46110)
    #[clap(name = "grpc")]
    grpc_proxy_address: Option<String>,

    // /// wRPC port
    /// interface:port for wRPC server (wrpc://127.0.0.1:47110)
    #[clap(long)]
...
async fn main() -> Result<()> {
    ...
    let sophisd_port = network_type.default_rpc_port();
    ...
    let options = Arc::new(Options {
        ...
        grpc_proxy_address: Some(grpc_proxy_address.unwrap_or_else(|| format!("grpc://127.0.0.1:{sophisd_port}"))),
        verbose,
        // ..Options::default()
    });
    log_info!("");
    ...
    let rpc_handler = Arc::new(SophisRpcHandler::new(tasks, encoding, None, options.clone()));
    ...
    log_info!("Sophis wRPC server is listening on {}", options.listen_address);
