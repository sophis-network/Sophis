//!
//! Module implementing [`Resolver`] client for obtaining public Sophis wRPC endpoints.
//!
...
///
/// # Resolver - a client for obtaining public Sophis wRPC endpoints.
///
/// This client operates against [Sophis Resolver](https://github.com/aspectron/sophis-resolver) service
/// that provides load-balancing and failover capabilities for Sophis wRPC endpoints. The default
/// configuration allows access to public Sophis nodes, while custom configurations can be supplied
/// if you are running your own custom Sophis node cluster.
///
#[derive(Debug, Clone)]
pub struct Resolver {
    inner: Arc<Inner>,
...
impl Resolver {
    ...
    fn make_url(&self, url: &str, encoding: Encoding, network_id: NetworkId) -> String {
        ...
        format!("{url}/v{CURRENT_VERSION}/sophis/{network_id}/{tls}/wrpc/{encoding}")
    }
    ...
    /// Obtain a Sophis p2p [`NodeDescriptor`] from the resolver based on the supplied [`Encoding`] and [`NetworkId`].
    ...
    /// Returns a Sophis wRPC URL from the resolver based on the supplied [`Encoding`] and [`NetworkId`].
