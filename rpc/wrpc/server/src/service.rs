...
use sophis_rpc_core::api::ops::RpcApiOps;
...
impl Default for Options {
    fn default() -> Self {
        Options { listen_address: "127.0.0.1:47110".to_owned(), verbose: false, grpc_proxy_address: None }
    }
}

/// ### SophisRpcHandler
///
/// [`SophisRpcHandler`] is a handler struct that implements the [`RpcHandler`] trait
...
///
pub struct SophisRpcHandler {
    pub server: Server,
    pub options: Arc<Options>,
}

impl SophisRpcHandler {
    pub fn new(
        tasks: usize,
        ...
        SophisRpcHandler { server: Server::new(tasks, encoding, core_service, options.clone()), options }
    }
}

#[async_trait]
impl RpcHandler for SophisRpcHandler {
    ...
    async fn handshake(
        ...
        //     sender,
        //     receiver,
        //     Box::pin(|msg| if msg != "sophis" { Err(WebSocketError::NegotiationFailure) } else { Ok(()) }),
        // )
        // .await
...
pub struct WrpcService {
    ...
    options: Arc<Options>,
    server: RpcServer,
    rpc_handler: Arc<SophisRpcHandler>,
...
impl WrpcService {
    ...
    pub fn new(
        ...
        let rpc_handler = Arc::new(SophisRpcHandler::new(tasks, *encoding, core_service, options.clone()));
