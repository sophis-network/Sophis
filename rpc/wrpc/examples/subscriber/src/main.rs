// Basic example of a Sophis wRPC client that connects to a node
...
// Sophis RPC primitives
use sophis_wrpc_client::prelude::*;
// reuse wRPC Result type for convenience
use sophis_wrpc_client::result::Result;
...
impl Listener {
    pub fn try_new(network_id: NetworkId, url: Option<String>) -> Result<Self> {
        ...
        // Create a basic Sophis RPC client instance using Borsh encoding.
        let client = Arc::new(SophisRpcClient::new_with_args(WrpcEncoding::Borsh, url.as_deref(), resolver, Some(network_id), None)?);
