...
use sophis_consensus_core::api::counters::ProcessingCounters;
use sophis_consensus_core::daa_score_timestamp::DaaScoreTimestamp;
use sophis_consensus_core::errors::block::RuleError;
use sophis_consensus_core::tx::{TransactionQueryResult, TransactionType};
use sophis_consensus_core::utxo::utxo_inquirer::UtxoInquirerError;
...
use sophis_index_core::indexed_utxos::BalanceByScriptPublicKey;
use sophis_index_core::{
...
use sophis_mining::feerate::FeeEstimateVerbose;
use sophis_mining::model::tx_query::TransactionQuery;
use sophis_mining::{manager::MiningManagerProxy, mempool::tx::Orphan};
use sophis_notify::listener::ListenerLifespan;
use sophis_notify::subscription::context::SubscriptionContext;
use sophis_notify::subscription::{MutationPolicies, UtxosChangedMutationPolicy};
use sophis_notify::{
...
use sophis_p2p_flows::flow_context::FlowContext;
use sophis_p2p_lib::common::ProtocolError;
use sophis_p2p_mining::rule_engine::MiningRuleEngine;
use sophis_perf_monitor::{Monitor as PerfMonitor, counters::CountersSnapshot};
use sophis_rpc_core::{
...
use sophis_txscript::{extract_script_pub_key_address, pay_to_address_script};
use sophis_utils::expiring_cache::ExpiringCache;
use sophis_utils::sysinfo::SystemInfo;
use sophis_utils::{channel::Channel, triggers::SingleTrigger};
use sophis_utils_tower::counters::TowerConnectionCounters;
use sophis_utxoindex::api::UtxoIndexProxy;
...
/// A service implementing the Rpc API at sophis_rpc_core level.
///
...
pub struct RpcCoreService {
    ...
    fee_estimate_verbose_cache: ExpiringCache<sophis_mining::errors::MiningManagerResult<GetFeeEstimateExperimentalResponse>>,
...
NOTE: This error usually indicates an RPC conversion error between the node and the miner. This is likely to reflect using a NON-SUPPORTED miner.",
    ...
    async fn get_block_template_call(
        ...
        if request.pay_address.prefix != self.config.prefix() {
            return Err(sophis_addresses::AddressError::InvalidPrefix(request.pay_address.prefix.to_string()))?;
        }
        ...
        let script_public_key = sophis_txscript::pay_to_address_script(&request.pay_address);
    ...
    async fn estimate_network_hashes_per_second_call(
        ...
        const LEGACY_VIRTUAL: sophis_hashes::Hash = sophis_hashes::Hash::from_bytes([0xff; sophis_hashes::HASH_SIZE]);
    ...
    async fn get_metrics_call(&self, _connection: Option<&DynRpcConnection>, req: GetMetricsRequest) -> RpcResult<GetMetricsResponse> {
        ...
        let consensus_metrics = if req.consensus_metrics {
            ...
            Some(ConsensusMetrics {
                ...
                // ---
                network_orphan_reds_count: processing_counters.mergeset_reds_total,
                network_orphan_blues_count: processing_counters.mergeset_blues_total,
            })
        } else {
            None
        };
    ...
    async fn get_da_payload_call(
        &self,
        _connection: Option<&DynRpcConnection>,
        request: GetDaPayloadRequest,
    ) -> RpcResult<GetDaPayloadResponse> {
        let payload_id = vec_to_array_48(&request.payload_id, "payload_id")?;
        let session = self.consensus_manager.consensus().unguarded_session();
        let entry = session.async_da_get_payload(payload_id).await;
        Ok(GetDaPayloadResponse::new(entry.map(payload_entry_to_rpc)))
    }

    async fn get_da_bundle_call(
        &self,
        _connection: Option<&DynRpcConnection>,
        request: GetDaBundleRequest,
    ) -> RpcResult<GetDaBundleResponse> {
        let bundle_id = vec_to_array_48(&request.bundle_id, "bundle_id")?;
        let session = self.consensus_manager.consensus().unguarded_session();
        let Some(index) = session.async_da_get_bundle_index(bundle_id).await else {
            return Ok(GetDaBundleResponse::new(None));
        };

        // Reassemble: read each fragment's PayloadEntry and concat data bytes
        // in fragment_index order (BundleIndex is already sorted on insert).
        let mut data_chunks: Vec<Vec<u8>> = Vec::with_capacity(index.payload_ids.len());
        let mut all_present = (index.payload_ids.len() as u8) == index.fragment_count;
        for pid in &index.payload_ids {
            if let Some(entry) = session.async_da_get_payload(pid.0).await {
                // The script bytes are <header><data>; data starts at offset CARRIER_HEADER_LEN.
                use sophis_consensus_core::da::CARRIER_HEADER_LEN;
                if entry.script.len() < CARRIER_HEADER_LEN {
                    all_present = false;
                    break;
                }
                let data_start = CARRIER_HEADER_LEN;
                let data_end = data_start + entry.data_len_from_script();
                if entry.script.len() < data_end {
                    all_present = false;
                    break;
                }
                data_chunks.push(entry.script[data_start..data_end].to_vec());
            } else {
                all_present = false;
                break;
            }
        }
        let payload_ids: Vec<Vec<u8>> = index.payload_ids.iter().map(|p| p.0.to_vec()).collect();
        let data = if all_present { Some(data_chunks.into_iter().flatten().collect()) } else { None };

        Ok(GetDaBundleResponse::new(Some(RpcDaBundle {
    ...
    async fn get_da_carriers_by_block_call(
        &self,
        _connection: Option<&DynRpcConnection>,
        request: GetDaCarriersByBlockRequest,
    ) -> RpcResult<GetDaCarriersByBlockResponse> {
        let session = self.consensus_manager.consensus().unguarded_session();
        let payload_ids = session.async_da_list_by_block(request.block_hash).await;
        Ok(GetDaCarriersByBlockResponse::new(payload_ids.into_iter().map(|p| p.to_vec()).collect()))
    }

    async fn get_da_carriers_by_domain_call(
        &self,
        _connection: Option<&DynRpcConnection>,
        request: GetDaCarriersByDomainRequest,
    ) -> RpcResult<GetDaCarriersByDomainResponse> {
        let session = self.consensus_manager.consensus().unguarded_session();
        let payload_ids = session.async_da_list_by_domain(request.domain_byte, request.blue_score).await;
        Ok(GetDaCarriersByDomainResponse::new(payload_ids.into_iter().map(|p| p.to_vec()).collect()))
    }

    async fn get_da_payload_status_call(
        &self,
        _connection: Option<&DynRpcConnection>,
        ...
        let payload_id = vec_to_array_48(&request.payload_id, "payload_id")?;
        let session = self.consensus_manager.consensus().unguarded_session();
        let Some(entry) = session.async_da_get_payload(payload_id).await else {
            return Ok(GetDaPayloadStatusResponse::new(None));
        };
        let current_blue_score = session.async_da_current_blue_score().await;
        let confirmations = current_blue_score.saturating_sub(entry.blue_score);
        Ok(GetDaPayloadStatusResponse::new(Some(RpcDaPayloadStatus {
            accepted: true,
            accepting_block_hash: entry.accepting_block_hash,
...
fn vec_to_array_48(v: &[u8], field: &str) -> RpcResult<[u8; 48]> {
    if v.len() != 48 {
        return Err(RpcError::General(format!("{field} must be 48 bytes (got {})", v.len())));
    }
    let mut arr = [0u8; 48];
...
fn payload_entry_to_rpc(entry: sophis_consensus_core::da::PayloadEntry) -> RpcDaPayload {
    // Recompute payload_id from the script for consumers that want it inline.
    let pid = sophis_consensus_core::da::payload_id(&entry.script);
    RpcDaPayload {
        payload_id: pid.to_vec(),
        script: entry.script,
        accepting_block_hash: entry.accepting_block_hash,
        blue_score: entry.blue_score,
        fragment_index: entry.fragment_index,
        fragment_count: entry.fragment_count,
        bundle_id: entry.bundle_id.0.to_vec(),
        domain_byte: entry.domain_byte,
    }
}

// Local extension to avoid re-exporting CARRIER_HEADER_LEN constants here.
trait PayloadEntryExt {
    fn data_len_from_script(&self) -> usize;
}
impl PayloadEntryExt for sophis_consensus_core::da::PayloadEntry {
    fn data_len_from_script(&self) -> usize {
        // Bytes 12..16 of the carrier script header are little-endian u32 data_len.
        if self.script.len() < 16 {
            return 0;
        }
        u32::from_le_bytes([self.script[12], self.script[13], self.script[14], self.script[15]]) as usize
    }
}
