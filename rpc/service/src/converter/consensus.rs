...
use sophis_consensus_notify::notification::{self as consensus_notify, Notification as ConsensusNotification};
use sophis_consensusmanager::{ConsensusManager, ConsensusProxy};
use sophis_hashes::Hash;
use sophis_math::Uint256;
use sophis_mining::model::{TransactionIdSet, owner_txs::OwnerTransactions};
use sophis_notify::converter::Converter;
use sophis_rpc_core::{
...
use sophis_txscript::{extract_script_pub_key_address, script_class::ScriptClass};
...
impl ConsensusConverter {
    ...
    ///
    /// _GO-SOPHISD: PopulateBlockWithVerboseData_
    ...
    ///
    /// _GO-SOPHISD: PopulateTransactionWithVerboseData
    ...
    fn get_utxo_verbose_data_with_verbosity(
        ...
        Ok(RpcOptionalUtxoEntryVerboseData {
            ...
            // Soft-skip non-addressable scripts (e.g. bridge UTXOs version 3/4/5
            // carry borsh payloads, not addresses). Symmetric with the V1 path
            // in `get_transaction_output` which uses `.ok()`. Hard-erroring here
            // would fail the entire V2 response whenever any bridge UTXO was
            // spent in the requested chain range.
    ...
    fn get_transaction_output_verbose_data_with_verbosity(
        ...
        Ok(RpcOptionalTransactionOutputVerboseData {
            ...
            // Soft-skip — see `get_utxo_verbose_data_with_verbosity` above for
            // the rationale (bridge UTXO outputs are not addressable).
            script_public_key_address: if verbosity.include_script_public_key_address.unwrap_or(false) {
