...
use regex::Regex;
use sophis_core::{debug, error, trace};
use sophis_grpc_core::{
    RPC_MAX_MESSAGE_SIZE,
    channel::NotificationChannel,
    ops::SophisdPayloadOps,
    protowire::{GetInfoRequestMessage, SophisdRequest, SophisdResponse, rpc_client::RpcClient, sophisd_request},
};
use sophis_notify::{
...
impl RpcApi for GrpcClient {
    ...
    //     self.inner.call(SophisdPayloadOps::SubmitBlock, request).await?.as_ref().try_into()
    // }
    ...
    // Phase 6 — Data Availability gRPC bindings (sub-fase 6.4.b)
    route!(get_da_payload_call, GetDaPayload);
    route!(get_da_bundle_call, GetDaBundle);
    route!(get_da_carriers_by_block_call, GetDaCarriersByBlock);
...
type SophisdRequestSender = async_channel::Sender<SophisdRequest>;
type SophisdRequestReceiver = async_channel::Receiver<SophisdRequest>;
...
/// Data flow:
/// ```
/// //   SophisdRequest -> request_send -> stream -> SophisdResponse
/// ```
///
/// Execution flow:
/// ```
...
struct Inner {
    ...
    // Sending to server
    request_sender: SophisdRequestSender,
    request_receiver: SophisdRequestReceiver,
...
impl Inner {
    ...
    async fn try_connect(
        ...
        request_timeout: u64,
        counters: Arc<TowerConnectionCounters>,
    ) -> Result<(Streaming<SophisdResponse>, ServerFeatures)> {
        // gRPC endpoint
        ...
        // Actual SophisdRequest to SophisdResponse stream
        let mut stream: Streaming<SophisdResponse> = client.message_stream(request_stream).await?.into_inner();
    ...
    async fn call(&self, op: SophisdPayloadOps, request: impl Into<SophisdRequest>) -> Result<SophisdResponse> {
        // Calls are only allowed if the client is connected to the server
        if self.is_connected() {
            let id = u64::from_le_bytes(rand::random::<[u8; 8]>());
            let mut request: SophisdRequest = request.into();
            request.id = id;
    ...
    fn spawn_response_receiver_task(self: Arc<Self>, mut stream: Streaming<SophisdResponse>) {
        // Note: self is a cloned Arc here so that it can be used in the spawned task.
    ...
    async fn start_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        let request = sophisd_request::Payload::from_notification_type(&scope, Command::Start);
    ...
    async fn stop_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        if self.handle_stop_notify() {
            let request = sophisd_request::Payload::from_notification_type(&scope, Command::Stop);
            self.call((&request).into(), request).await?;
        }
        Ok(())
    }
