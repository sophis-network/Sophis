use crate::{
    error::{Error, Result},
    resolver::{Resolver, SophisdResponseReceiver, SophisdResponseSender, matcher::Matcher},
};
...
struct Pending {
    ...
    sender: SophisdResponseSender,
}

impl Pending {
    fn new(op: SophisdPayloadOps, request: SophisdRequest, sender: SophisdResponseSender) -> Self {
        Self { timestamp: Instant::now(), op, request, sender }
    }

    fn is_matching(&self, response: &SophisdResponse, response_op: SophisdPayloadOps) -> bool {
...
impl Resolver for QueueResolver {
    fn register_request(&self, op: SophisdPayloadOps, request: &SophisdRequest) -> SophisdResponseReceiver {
        let (sender, receiver) = oneshot::channel::<Result<SophisdResponse>>();
        {
    ...
    fn handle_response(&self, response: SophisdResponse) {
        let response_op: SophisdPayloadOps = response.payload.as_ref().unwrap().try_into().expect("response is not a notification");
