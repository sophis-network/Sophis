...
use sophis_hashes::MerkleHash;
use sophis_rpc_core::{FromRpcHex, RpcError, RpcHash, RpcResult, ToRpcHex};
use std::str::FromStr;
...
from!(item: &sophis_rpc_core::RpcHeader, protowire::RpcBlockHeader, {
    Self {
...
from!(item: &sophis_rpc_core::RpcRawHeader, protowire::RpcBlockHeader, {
    Self {
...
try_from!(item: &protowire::RpcBlockHeader, sophis_rpc_core::RpcHeader, {
    ...
    let header = Header::new_finalized(
        ...
        MerkleHash::from_str(&item.hash_merkle_root)?,
        MerkleHash::from_str(&item.accepted_id_merkle_root)?,
        RpcHash::from_str(&item.utxo_commitment)?,
        item.timestamp.try_into()?,
        item.bits,
        item.nonce,
        item.daa_score,
        sophis_rpc_core::RpcBlueWorkType::from_rpc_hex(&item.blue_work)?,
        item.blue_score,
...
try_from!(item: &protowire::RpcBlockHeader, sophis_rpc_core::RpcRawHeader, {
    Self {
        ...
        hash_merkle_root: MerkleHash::from_str(&item.hash_merkle_root)?,
        accepted_id_merkle_root: MerkleHash::from_str(&item.accepted_id_merkle_root)?,
        utxo_commitment: RpcHash::from_str(&item.utxo_commitment)?,
        timestamp: item.timestamp.try_into()?,
        bits: item.bits,
        nonce: item.nonce,
        daa_score: item.daa_score,
        blue_work: sophis_rpc_core::RpcBlueWorkType::from_rpc_hex(&item.blue_work)?,
        blue_score: item.blue_score,
        pruning_point: RpcHash::from_str(&item.pruning_point)?,
    }
});

try_from!(item: &protowire::RpcBlockHeader, sophis_rpc_core::RpcOptionalHeader, {
    // We re-hash the block to remain as most trustless as possible
    let header = Header::new_finalized(
        item.version.try_into()?,
        item.parents.iter().map(Vec::<RpcHash>::try_from).collect::<RpcResult<Vec<Vec<RpcHash>>>>()?.try_into()?,
    ...
    sophis_rpc_core::RpcOptionalHeader::from(header)
});
...
mod tests {
    ...
    use sophis_consensus_core::{block::Block, header::Header};
    use sophis_hashes::MerkleHash;
    use sophis_rpc_core::{RpcBlock, RpcHash, RpcHeader};
    ...
    fn new_unique_merkle() -> MerkleHash {
        use std::sync::atomic::{AtomicU64, Ordering};
        static COUNTER: AtomicU64 = AtomicU64::new(1);
        let c = COUNTER.fetch_add(1, Ordering::Relaxed);
