...
fn compressed_parents_to_protowire(parents: &sophis_rpc_core::RpcCompressedParents) -> Vec<protowire::RpcBlockLevelRun> {
    parents
        .raw()
        .iter()
...
fn compressed_parents_from_protowire(runs: &[protowire::RpcBlockLevelRun]) -> RpcResult<sophis_rpc_core::RpcCompressedParents> {
...
from!(item: &sophis_rpc_core::RpcOptionalHeader, protowire::RpcOptionalHeader, {
    Self {
...
try_from!(item: &protowire::RpcOptionalHeader, sophis_rpc_core::RpcOptionalHeader, {
    Self {
        ...
        hash_merkle_root: item.hash_merkle_root.as_ref().map(|x| MerkleHash::from_str(x)).transpose()?,
        accepted_id_merkle_root: item.accepted_id_merkle_root.as_ref().map(|x| MerkleHash::from_str(x)).transpose()?,
        utxo_commitment: item.utxo_commitment.as_ref().map(|x| RpcHash::from_str(x)).transpose()?,
        timestamp: item.timestamp.map(|x| x as u64),
        bits: item.bits,
        nonce: item.nonce,
        daa_score: item.daa_score,
        blue_work: item.blue_work.as_ref().map(|x| sophis_rpc_core::RpcBlueWorkType::from_rpc_hex(x)).transpose()?,
        blue_score: item.blue_score,
