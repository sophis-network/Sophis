...
use sophis_notify::scope::{NewBlockTemplateScope, Scope};
use sophis_rpc_core::api::rpc::RpcApi;
use sophis_utils::networking::{ContextualNetAddress, NetAddress};
use std::sync::Arc;

#[tokio::test]
async fn test_client_server_sanity_check() {
    sophis_core::log::try_init_logger("info, sophis_grpc_core=trace, sophis_grpc_server=trace, sophis_grpc_client=trace");
