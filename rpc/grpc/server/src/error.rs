...
pub enum GrpcServerError {
    ...
    RpcApiError(#[from] sophis_rpc_core::error::RpcError),
    ...
    ClosedHandler(SophisdPayloadOps),
...
impl From<GrpcServerError> for sophis_rpc_core::error::RpcError {
    fn from(err: GrpcServerError) -> Self {
        match err {
            GrpcServerError::RpcApiError(err) => err,
            GrpcServerError::NotificationError(err) => err.into(),
            _ => sophis_rpc_core::error::RpcError::General(err.to_string()),
        }
    }
}

impl From<GrpcServerError> for sophis_notify::error::Error {
    fn from(err: GrpcServerError) -> Self {
        match err {
            GrpcServerError::RpcApiError(err) => sophis_notify::error::Error::General(err.to_string()),
            GrpcServerError::NotificationError(err) => err,
            _ => sophis_notify::error::Error::General(err.to_string()),
        }
    }
}
...
impl<T> From<TrySendError<T>> for GrpcServerError {
    fn from(_: TrySendError<T>) -> Self {
        sophis_notify::error::Error::ChannelSendError.into()
    }
}
