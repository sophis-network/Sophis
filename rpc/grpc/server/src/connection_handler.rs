...
use sophis_grpc_core::{
    ...
    protowire::{
        SophisdRequest, SophisdResponse,
        ...
        let server_handle = tokio::spawn(async move {
            ...
            let serve_result = TonicServer::builder()
                ...
                .layer(MapRequestBodyLayer::new(move |body| {
                    tonic::body::Body::new(CountBytesBody::new(body, bytes_rx.clone()))
                }))
                .layer(MapResponseBodyLayer::new(move |body| {
                    tonic::body::Body::new(CountBytesBody::new(body, bytes_tx.clone()))
                }))
...
impl Rpc for ConnectionHandler {
    type MessageStreamStream = Pin<Box<dyn Stream<Item = Result<SophisdResponse, tonic::Status>> + Send + Sync + 'static>>;

    /// Handle the new arriving client connection
    async fn message_stream(
        &self,
        request: Request<tonic::Streaming<SophisdRequest>>,
