# Sophis — documentation index

Single navigable table of contents for the ~80 markdown files in this
...
> Last refreshed: 2026-05-11 (post-Phase 5 deprecation cleanup).
...
- [wallet/descriptors/DESIGN.md](wallet/descriptors/DESIGN.md) — output script descriptors
...
If you add a new `.md`, add a one-line entry here under the matching
audience header. Keep entries to a single line with a short purpose
clause after the em-dash. The goal is `Ctrl+F` navigability — not
comprehensive prose. Anything longer than one line belongs in the
file itself.
