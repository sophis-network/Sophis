...
impl Default for SophisdConfig {
    fn default() -> Self {
        Self {
            mute: false,
            path: None,
...
impl TryFrom<SophisdConfig> for Vec<String> {
    type Error = Error;
    fn try_from(args: SophisdConfig) -> Result<Vec<String>> {
        let mut argv = Vec::new();

        if args.path.is_none() {
            return Err(Error::Custom("no sophisd path is specified".to_string()));
        }
...
impl Sophisd {
    pub fn new(args: SophisdConfig) -> Self {
        Self {
    ...
    pub fn configure(&self, config: SophisdConfig) -> Result<()> {
    ...
    pub fn start(&self) -> Result<()> {
        ...
        if let Some(process) = process
            && process.is_running()
        {
            return Err(Error::Custom("Sophis node is already running.".to_string()));
        }
        ...
        // let options = SophisdOptions::new(path,network)?;
    ...
    pub async fn version(&self) -> Result<String> {
        ...
        if let Some(path) = path {
            ...
            Ok("Sophisd binary is not configured. Please use 'node select' command.".to_string())
        }
    }
}

#[async_trait]
pub trait SophisdCtl {
    async fn version(&self) -> Result<String>;
    async fn configure(&self, config: SophisdConfig) -> Result<()>;
