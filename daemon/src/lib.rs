...
pub use sophisd::{Sophisd, SophisdConfig, SophisdCtl};
...
pub static LOCATIONS: &[&str] = &[
    ...
    "../target/release",
    "../target/debug",
    "../../sophis-cpu-miner/target/debug",
    "../../sophis-cpu-miner/target/release",
    "bin/windows-x64",
    "bin/linux-ia32",
    "bin/linux-x64",
    "bin/linux-arm64",
...
#[derive(Default)]
pub struct Daemons {
    pub sophisd: Option<Arc<dyn SophisdCtl + Send + Sync + 'static>>,
    // pub sophisd_automute : Arc<
    pub cpu_miner: Option<Arc<dyn CpuMinerCtl + Send + Sync + 'static>>,
}

impl Daemons {
    pub fn new() -> Self {
        Self { sophisd: None, cpu_miner: None }
    }

    pub fn with_sophisd(mut self, sophisd: Arc<dyn SophisdCtl + Send + Sync + 'static>) -> Self {
        self.sophisd = Some(sophisd);
        self
    }
    ...
    pub fn sophisd(&self) -> Arc<dyn SophisdCtl + Send + Sync + 'static> {
        self.sophisd.as_ref().expect("accessing Daemons::sophisd while sophisd option is None").clone()
    }

    pub fn try_sophisd(&self) -> Option<Arc<dyn SophisdCtl + Send + Sync + 'static>> {
        self.sophisd.clone()
    }
