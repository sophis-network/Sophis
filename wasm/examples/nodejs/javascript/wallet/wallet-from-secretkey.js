...
const fs = require('fs');
const sophis = require('../../../../nodejs/sophis-dev');
const {
...
(async()=>{
    ...
    async function log_transactions(accountId){
        ...
        transactionsResult.transactions.forEach((tx)=>{
            ...
            list.push({
                Id: tx.id,
                Type: tx.data.type,
                Value: sompiToSophisString(value(tx)||0)
            });
    ...
    try {
                ...
                case "balance":
                    ...
                    Object.keys(balance).map(id=>{
                        ...
                        list.push({
                            ...
                            Mature: sompiToSophisString(b.mature),
                            Pending: sompiToSophisString(b.pending),
                            Outgoing: sompiToSophisString(b.outgoing),
        ...
        // Send sophis to address
        let sendResult = await wallet.accountsSend({
            walletSecret,
            // @ts-ignore
            accountId: firstAccount.accountId,
            priorityFeeSompi: sophisToSompi("0.001"),
            destination:[{
                // @ts-ignore
                address: firstAccount.changeAddress,
                amount: sophisToSompi("1.567")
            }]
        });
        ...
        // Transfer sophis between accounts
        let transferResult = await wallet.accountsTransfer({
            ...
            transferAmountSompi: sophisToSompi("2.456"),
        });
