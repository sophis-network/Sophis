...
## WASM32 bindings for Rusty Sophis SDK
...
- [**Rust** documentation](https://docs.rs/sophis-wasm/latest/sophis_wasm/index.html)
- [**TypeScript** documentation](https://sophis.aspectron.org/docs/)
...
- **RPC API** — RPC API for the Sophis node using WebSockets.
- **Wallet SDK** — Bindings for primitives related to key management and transactions.
- **Wallet API** — API for the Rusty Sophis Wallet framework.
...
- `web/sophis` - **full** Rusty Sophis WASM32 SDK bindings for use in web browsers.
- `web/sophis-rpc` - only the RPC bindings for use in web browsers (reduced WASM binary size).
- `nodejs/sophis` - **full** Rusty Sophis WASM32 SDK bindings for use with NodeJS.
- `docs` - Rusty Sophis WASM32 SDK documentation.
...
- `examples/javascript/wallet` - Interfacing with the Rusty Sophis Wallet framework.
...
Included documentation in the release can be accessed by loading the `docs/sophis/index.html` 
file in a web browser.

## Building from Source

To build the WASM32 SDK from source, you need to have the Rust environment installed. To do that,
follow instructions in the [Rusty Sophis README](https://github.com/sophisnet/rusty-sophis).
...
serve them from the root of the SDK folder (`sophis-wasm32-sdk` if using a redistributable or
`rusty-sophis/wasm` if building from source). This is because examples use relative paths.
...
before loading the WASM32 library. The compatible WebSocket library is [WebSocket](https://www.npmjs.com/package/websocket) and is included in the `sophis` NPM package. `sophis` package is a wrapper around `sophis-wasm` that imports and installs this WebSocket shim in the `globalThis` object and then re-exports `sophis-wasm` exports.
...
<html>
    <head>
        <script type="module">
            import * as sophis from './sophis/sophis-wasm.js';
            (async () => {
                await sophis.default('./sophis/sophis-wasm_bg.wasm');
