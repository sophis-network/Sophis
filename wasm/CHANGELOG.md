...
- Merge with Rusty Sophis master (0.13.6)
...
- Rename `sophis-beacon` app to `sophis-resolver`
...
- Introduced `Beacon` class that provides connectivity to the community-operated public node infrastructure (backed by `sophis-beacon` load balancer & node status monitor)
...
- Changed documentation structure to use `typedoc` available as a part of redistributables or online at https://sophis.aspectron.org/docs/
