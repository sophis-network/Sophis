...
use sophis_addressmanager::NetAddress;
use sophis_p2p_lib::{
    IncomingRoute, Router,
    common::ProtocolError,
    dequeue, dequeue_with_timeout, make_message,
    pb::{AddressesMessage, RequestAddressesMessage, sophisd_message::Payload},
};
use sophis_utils::networking::IpAddress;
use std::sync::Arc;

/// The maximum number of addresses that are sent in a single sophis Addresses message.
const MAX_ADDRESSES_SEND: usize = 1000;

/// The maximum number of addresses that can be received in a single sophis Addresses response.
...
impl SendAddressesFlow {
    ...
    async fn start_impl(&mut self) -> Result<(), ProtocolError> {
        loop {
            ...
            let address_list =
                addresses.choose_multiple(&mut rand::rng(), MAX_ADDRESSES_SEND).map(|addr| (addr.ip, addr.port).into()).collect();
