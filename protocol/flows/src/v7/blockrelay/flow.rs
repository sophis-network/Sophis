...
use sophis_consensus_core::{api::BlockValidationFutures, block::Block, blockstatus::BlockStatus, errors::block::RuleError};
use sophis_consensusmanager::{BlockProcessingBatch, ConsensusProxy};
...
use sophis_p2p_lib::{
    ...
    pb::{InvRelayBlockMessage, RequestBlockLocatorMessage, RequestRelayBlocksMessage, sophisd_message::Payload},
};
use sophis_utils::channel::{JobSender, JobTrySendError as TrySendError};
...
impl HandleRelayInvsFlow {
    ...
    async fn check_orphan_resolution_range(
        ...
        // with current syncer-side implementations (in both go-sophis and this codebase) we could query only the last one,
