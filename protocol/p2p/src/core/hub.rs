use crate::{ConnectionInitializer, Peer, Router, common::ProtocolError, pb::SophisdMessage};
...
impl Hub {
    ...
    pub async fn send(&self, peer_key: PeerKey, msg: SophisdMessage) -> Result<bool, ProtocolError> {
    ...
    pub async fn broadcast(&self, msg: SophisdMessage, filter_peer: Option<PeerKey>) {
        let peers = self
            .peers
            .read()
            .values()
    ...
    pub async fn broadcast_to_some_peers(&self, msg: SophisdMessage, num_peers: usize) {
        assert!(num_peers > 0);
    ...
    pub async fn broadcast_many(&self, msgs: Vec<SophisdMessage>, filter_peer: Option<PeerKey>) {
        if msgs.is_empty() {
            return;
        }
        let peers = self
