...
use crate::pb::{ReadyMessage, VerackMessage, VersionMessage, sophisd_message::Payload};
use crate::{IncomingRoute, Router, SophisdMessagePayloadType};
use crate::{common::ProtocolError, dequeue_with_timeout, make_message};
use sophis_core::debug;

/// Implements the Sophis peer-to-peer handshake protocol
pub struct SophisdHandshake<'a> {
    router: &'a Router,
...
impl<'a> SophisdHandshake<'a> {
    /// Builds the handshake object and subscribes to handshake messages
    pub fn new(router: &'a Router) -> Self {
        Self {
            router,
            version_receiver: router.subscribe(vec![SophisdMessagePayloadType::Version]),
            verack_receiver: router.subscribe(vec![SophisdMessagePayloadType::Verack]),
            ready_receiver: router.subscribe(vec![SophisdMessagePayloadType::Ready]),
        }
    }
