...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for (p, (n, b)) in &counts {
        ...
        tot_n += n; tot_b += b;
        if NAMED.iter().any(|(pp, _)| pp == p) { da_n += n; da_b += b; }
    }
    println!("---");
    ...
    println!("All prefixes total:                     n={}, MB={:.1} (logical k+v; physical SST is larger due to write-amp/headers)", tot_n, tot_b as f64 / (1024.0 * 1024.0));
