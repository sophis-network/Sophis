...
use sophis_consensusmanager::{ConsensusProxy, spawn_blocking};
use sophis_core::{debug, error, info, time::Stopwatch, warn};
use sophis_mining_errors::{manager::MiningManagerError, mempool::RuleError};
use std::sync::Arc;
...
impl MiningManager {
                ...
                Err(BuilderError::ConsensusError(BlockRuleError::InvalidTransactionsInNewBlock(invalid_transactions))) => {
                    ...
                    invalid_transactions.iter().for_each(|(x, err)| {
                        ...
                        if let Err(err) = removal_result {
                            ...
                            // https://github.com/sophisnet/sophisd/issues/1553
    ...
    pub(crate) fn get_realtime_feerate_estimations_verbose(
        ...
        {
            let script_public_key = sophis_txscript::pay_to_address_script(&sophis_addresses::Address::new(
                prefix,
            ...
            let BlockTemplate { block: sophis_consensus_core::block::MutableBlock { transactions, .. }, calculated_fees, .. } =
...
mod tests {
    use super::*;
    use sophis_consensus_core::subnets;
