...
impl Frontier {
    ...
    ///           from `~1/50` (2000 sompi for a transaction with 100K storage mass), to `5M` (100 SPHS fee for a transaction with
        ...
        R: Rng + ?Sized,
    {
        ...
        let mut distr = Uniform::new(0f64, self.total_weight()).expect("uniform: valid range");
            ...
            let item = {
                ...
                while !cache.insert(item.tx.id()) {
                    ...
                    if cache.contains(&top.tx.id()) {
                        ...
                        distr = Uniform::new(0f64, remaining_weight).expect("uniform: valid range");
                    }
                    let query = distr.sample(rng);
