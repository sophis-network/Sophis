...
use sophis_consensus_core::{hashing::sighash::SigHashReusedValuesUnsync, mass::NonContextualMasses};
use sophis_txscript::{get_sig_op_count_upper_bound, is_unspendable, script_class::ScriptClass};
...
/// Increased from 100_000 to accommodate Dilithium-2 TXs whose storage mass can reach ~1_000_000.
/// Set equal to max_block_mass (10_000_000) so any TX that fits in a block also passes standardness.
const MAXIMUM_STANDARD_TRANSACTION_MASS: u64 = 10_000_000;
...
impl Mempool {
    pub(crate) fn check_transaction_standard_in_isolation(&self, transaction: &MutableTransaction) -> NonStandardResult<()> {
        ...
        // However, in a later version of Sophis the consensus-valid transaction version range might diverge from the
        ...
        for (i, output) in transaction.tx.outputs.iter().enumerate() {
            // Versions reserved by the Phase 3 ZK-Rollup internal bridge (vault=3, claim=4)
            // and the Phase 6 DA carrier (v=5) carry borsh / opaque payloads instead of
            // executable scripts. They are accepted as standard with the regular SPK
            // version cap and skip the dust + non-standard-class checks.
            let v = output.script_public_key.version();
            let is_bridge_version = matches!(v, 3 | 4);
            let is_carrier_version = v == sophis_consensus_core::constants::SCRIPT_VERSION_CARRIER;
            let is_protocol_payload = is_bridge_version || is_carrier_version;
            if v > MAX_SCRIPT_PUBLIC_KEY_VERSION {
                return Err(NonStandardError::RejectScriptPublicKeyVersion(transaction_id, i));
            }

            // Defense in depth: carriers (v=5) MUST parse as a well-formed
            ...
            if !is_protocol_payload && ScriptClass::from_script(&output.script_public_key) == ScriptClass::NonStandard {
                return Err(NonStandardError::RejectOutputScriptClass(transaction_id, i));
            }

            // Rollup UTXOs (vault=3, claim=4) carry borsh-encoded payloads and the
            // Phase 6 carrier (v=5) carries opaque DA bytes with mandatory value == 0;
            // `is_unspendable` flags them as such, which auto-classifies any value as
            // dust. The protocol consumes them via dedicated flows, not signatures, so
            // the dust check is meaningless and skipped here.
            if !is_protocol_payload && self.is_transaction_output_dust(output) {
...
mod tests {
    ...
    fn test_calc_min_required_tx_relay_fee() {
        ...
        let tests = [
            ...
            Test {
                ...
                minimum_relay_transaction_fee: DEFAULT_MINIMUM_RELAY_TRANSACTION_FEE,
                want: MAXIMUM_STANDARD_TRANSACTION_MASS * DEFAULT_MINIMUM_RELAY_TRANSACTION_FEE / 1000,
            },
    ...
    fn test_check_transaction_standard_in_isolation() {
        ...
        let dummy_prev_out = TransactionOutpoint::new(sophis_hashes::Hash::from_u64_word(1), 1);
        ...
        let addr = Address::new(Prefix::Testnet, Version::PubKeyDilithium, &addr_hash);
        let dummy_script_public_key = sophis_txscript::pay_to_address_script(&addr);
        let dummy_tx_out = TransactionOutput::new(SOMPI_PER_SOPHIS, dummy_script_public_key);
