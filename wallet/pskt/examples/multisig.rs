...
use sophis_consensus_core::config::params::TESTNET_PARAMS;
use sophis_consensus_core::{
    hashing::sighash::{SigHashReusedValuesUnsync, calc_signature_hash},
    tx::{TransactionId, TransactionOutpoint, UtxoEntry},
};
use sophis_txscript::{multisig_redeem_script, opcodes::codes::OpData65, pay_to_script_hash_script, script_builder::ScriptBuilder};
use sophis_wallet_pskt::prelude::{
        ...
        signer_pskt
            .pass_signature_sync(|tx, sighash| -> Result<Vec<SignInputOk>, String> {
                ...
                tx.tx
                    ...
                    .enumerate()
                    .map(|(idx, _input)| {
                        let hash = calc_signature_hash(&tx.as_verifiable(), idx, sighash[idx], &reused_values);
