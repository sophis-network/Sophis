...
After starting the wallet shell (native or WASM) and starting a local rusty-sophis Sophisd node (with `--testnet` and `--utxoindex`), you should perform the following actions:
```
network testnet
server localhost
connect
