...
use sha1::Sha1;
use sophis_bip32::{ExtendedPrivateKey, Language, Mnemonic, Prefix, SecretKey};
...
// ---
// {"type":"sophis-wallet","encryption":"default","version":1,"generator":"pwa","wallet":{"mnemonic":"hex"}}
...
fn legacy_v0_keydata_location() -> Result<(PathBuf, Options)> {
    let filename = if runtime::is_windows() {
        ...
        fs::resolve_path(&format!("{appdata}/Sophis/sophis.kpk"))?
    } else if runtime::is_macos() {
        fs::resolve_path("~/Library/Application Support/Sophis/sophis.kpk")?
    } else {
        fs::resolve_path("~/.sophis/sophis.kpk")?
    };

    let options = workflow_store::fs::Options::with_local_storage_key("sophis-wallet");
