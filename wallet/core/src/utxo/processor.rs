...
use sophis_wrpc_client::SophisRpcClient;
...
impl UtxoProcessor {
    ...
    pub fn rpc_client(&self) -> Option<Arc<SophisRpcClient>> {
        self.rpc_api().clone().downcast_arc::<SophisRpcClient>().ok()
    }
    ...
    pub async fn init_state_from_server(&self) -> Result<bool> {
        ...
        log_trace!("Connected to sophisd: '{server_version}' on '{server_network_id}';  SYNC: {is_synced}  DAA: {virtual_daa_score}");
