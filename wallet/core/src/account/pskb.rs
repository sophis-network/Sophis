...
//! (Partial Signed Sophis Transaction Bundles).
//!
...
use sophis_bip32::{DerivationPath, KeyFingerprint, PrivateKey};
use sophis_consensus_client::UtxoEntry as ClientUTXO;
use sophis_consensus_core::hashing::sighash::{SigHashReusedValuesUnsync, calc_signature_hash};
use sophis_consensus_core::tx::VerifiableTransaction;
use sophis_consensus_core::tx::{TransactionInput, UtxoEntry};
use sophis_txscript::extract_script_pub_key_address;
use sophis_txscript::opcodes::codes::OpData65;
use sophis_txscript::script_builder::ScriptBuilder;
use sophis_wallet_core::tx::{Generator, GeneratorSettings, PaymentDestination, PendingTransaction};
pub use sophis_wallet_pskt::bundle::Bundle;
...
pub fn pskt_to_pending_transaction(
    ...
    let output: &Vec<sophis_consensus_core::tx::TransactionOutput> = &signed_tx.outputs;
    if output.is_empty() {
    ...
    let pending_tx = PendingTransaction::try_new(
        ...
        0,
        0,
        sophis_wallet_core::tx::DataKind::Final,
    )?;
