...
use sophis_consensus_core::tx::{SignableTransaction, Transaction, TransactionId, TransactionInput, TransactionOutput};
use sophis_rpc_core::{RpcTransaction, RpcTransactionId};
...
impl PendingTransaction {
    ...
    pub fn create_input_signature(&self, _input_index: usize, _private_key: &[u8; 32], _hash_type: SigHashType) -> Result<Vec<u8>> {
        // Dilithium signing requires 2560-byte ML-DSA-44 keys; use sign_input_dilithium from consensus/core.
        // This method is kept for API compatibility — callers should use PendingTransaction::sign_input instead.
        Err(crate::error::Error::Custom("create_input_signature: use Dilithium signing via sign_input_dilithium".to_string()))
    }
    ...
    pub fn sign_input(&self, _input_index: usize, _private_key: &[u8; 32], _hash_type: SigHashType) -> Result<()> {
        // Dilithium signing requires 2560-byte ML-DSA-44 keys; secp256k1/Schnorr removed.
        // Use the wallet's Dilithium signer flow instead.
        Err(crate::error::Error::Custom("sign_input: use Dilithium signing via the wallet signer".to_string()))
    }

    pub fn try_sign_with_keys(&self, _privkeys: &[[u8; 32]], _check_fully_signed: Option<bool>) -> Result<()> {
        // Schnorr/ECDSA multi-key signing removed. Use the Dilithium wallet signer.
        Err(crate::error::Error::Custom("try_sign_with_keys: Schnorr/ECDSA removed; use Dilithium wallet signer".to_string()))
    }
