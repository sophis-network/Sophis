...
use sophis_consensus_core::tx::SignableTransaction;
...
impl SignerT for Signer {
    fn try_sign(&self, mutable_tx: SignableTransaction, addresses: &[Address]) -> Result<SignableTransaction> {
        ...
        // TODO: implement Dilithium multi-key signing (ML-DSA-44)
        // Schnorr/ECDSA signing removed; wallet signer integration pending.
        let _ = keys_for_signing.as_slice();
        keys_for_signing.zeroize();
        // Return transaction unsigned for now — Dilithium signing happens at a higher layer
        Ok(mutable_tx)
    }
}
...
impl KeydataSigner {
    pub fn new(keydata: Vec<(Address, [u8; 32])>) -> Self {
        let keys = keydata.into_iter().collect();
