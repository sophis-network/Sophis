...
impl From<Sophis> for Sompi {
    fn from(sophis: Sophis) -> Self {
        Sompi(sophis_to_sompi(sophis.0))
    }
}
    ...
    F: FnOnce(NetworkType) -> Address,
{
    ...
    let utxo_entries: Vec<UtxoEntryReference> = values.into_iter().map(sophis_to_sompi).map(UtxoEntryReference::simulated).collect();
...
// Test addresses are generated from fixed payloads via Address::new (avoids
// hardcoded bech32 strings that were leftovers from Kaspa with stale checksums).
pub(crate) fn change_address(network_type: NetworkType) -> Address {
    ...
    Address::new(prefix, Version::PubKeyDilithium, &[0xaa; 32])
}
...
pub(crate) fn output_address(network_type: NetworkType) -> Address {
    ...
    Address::new(prefix, Version::PubKeyDilithium, &[0xbb; 32])
}
...
fn test_generator_sweep_two_utxos() -> Result<()> {
    make_generator(test_network_id(), &[10.0, 10.0], &[], None, Fees::None, change_address, PaymentDestination::Change)
        ...
        .fetch(&Expected {
            is_final: true,
            input_count: 2,
            aggregate_input_value: Sophis(20.0),
            output_count: 1,
...
#[test]
fn test_generator_compound_200k_10sphs_transactions() -> Result<()> {
    generator(
        test_network_id(),
        &[10.0; 200_000],
        &[],
        None,
        Fees::sender(Sophis(5.0)),
        [(output_address, Sophis(190_000.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .validate()
...
#[test]
fn test_generator_fee_rate_compound_200k_10sphs_transactions() -> Result<()> {
    generator(
        test_network_id(),
        &[10.0; 200_000],
        &[],
...
fn test_generator_random_outputs() -> Result<()> {
    ...
    generator(test_network_id(), &[total + 100.0], &[], None, Fees::sender(Sophis(5.0)), outputs.as_slice())
        .unwrap()
        .harness()
        .validate()
        .finalize();
...
fn test_generator_dust_1_1() -> Result<()> {
    generator(
        ...
        None,
        Fees::sender(Sophis(5.0)),
        [(output_address, Sophis(1.0)), (output_address, Sophis(1.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
        is_final: true,
        input_count: 4,
        aggregate_input_value: Sophis(40.0),
        output_count: 3,
        priority_fees: FeesExpected::sender(Sophis(5.0)),
    })
    .finalize();
...
fn test_generator_inputs_2_outputs_2_fees_exclude() -> Result<()> {
    generator(
        ...
        None,
        Fees::sender(Sophis(5.0)),
        [(output_address, Sophis(10.0)), (output_address, Sophis(1.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
...
#[test]
#[ignore = "TODO Sophis: hardcoded expected aggregate_input assumes Schnorr-era mass; recalibrate for Dilithium 1312-byte VK signature script"]
fn test_generator_inputs_100_outputs_1_fees_exclude_success() -> Result<()> {
    // generator(test_network_id(), &[10.0; 100], &[], Fees::sender(Sophis(5.0)), [(output_address, Sophis(990.0))].as_slice())
    generator(test_network_id(), &[10.0; 100], &[], None, Fees::sender(Sophis(0.0)), [(output_address, Sophis(990.0))].as_slice())
        .unwrap()
        .harness()
        .fetch(&Expected {
            is_final: false,
            input_count: 88,
            aggregate_input_value: Sophis(880.0),
            output_count: 1,
            priority_fees: FeesExpected::None,
        })
        .fetch(&Expected {
            is_final: false,
            input_count: 12,
            aggregate_input_value: Sophis(120.0),
            output_count: 1,
            priority_fees: FeesExpected::None,
        })
        .fetch(&Expected {
            ...
            output_count: 2,
            // priority_fees: FeesExpected::sender(Sophis(5.0)),
            priority_fees: FeesExpected::sender(Sophis(0.0)),
        })
        .finalize();
...
fn test_generator_inputs_100_outputs_1_fees_include_success() -> Result<()> {
    generator(
        ...
        &[],
        None,
        Fees::receiver(Sophis(5.0)),
        // [(output_address, Sophis(100.0))].as_slice(),
        [(output_address, Sophis(100.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
        is_final: false,
        input_count: 88,
        aggregate_input_value: Sophis(88.0),
        output_count: 1,
        priority_fees: FeesExpected::None,
    })
    .fetch(&Expected {
        is_final: false,
        input_count: 12,
        aggregate_input_value: Sophis(12.0),
        output_count: 1,
        priority_fees: FeesExpected::None,
    })
    .fetch(&Expected {
        ...
        output_count: 1,
        priority_fees: FeesExpected::receiver(Sophis(5.0)),
    })
    .finalize();
...
fn test_generator_inputs_100_outputs_1_fees_exclude_insufficient_funds() -> Result<()> {
    generator(test_network_id(), &[10.0; 100], &[], None, Fees::sender(Sophis(5.0)), [(output_address, Sophis(1000.0))].as_slice())
        .unwrap()
        .harness()
        .fetch(&Expected {
            is_final: false,
...
fn test_generator_inputs_1k_outputs_2_fees_exclude() -> Result<()> {
    generator(test_network_id(), &[10.0; 1_000], &[], None, Fees::sender(Sophis(5.0)), [(output_address, Sophis(9_000.0))].as_slice())
        .unwrap()
        .harness()
        .drain(
            10,
        ...
        .fetch(&Expected {
            is_final: false,
            input_count: 21,
            aggregate_input_value: Sophis(210.0),
            output_count: 1,
            priority_fees: FeesExpected::None,
        })
        .fetch(&Expected {
...
fn test_generator_inputs_32k_outputs_2_fees_exclude() -> Result<()> {
    ...
    generator(
        ...
        &[],
        None,
        Fees::sender(Sophis(10_000.0)),
        [(output_address, Sophis(f * 32_747.0 - 10_001.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .accumulate(379)
...
fn test_generator_fan_out_1() -> Result<()> {
    use sophis_consensus_core::mass::calc_storage_mass;
    ...
    //     1.00000000,
    //     87.23579967,
    // ], &[], None, Fees::sender(Sophis(1.0)), [
    //     (output_address, Sophis(0.20000000)),
    //     (output_address, Sophis(0.25000000)),
    //     (output_address, Sophis(0.21000000)),
    // ].as_slice())
    //     .unwrap()
    //     .harness()
    //     // .accumulate(1)
    ...
    //         is_final: true,
    //         input_count: 2,
    //         aggregate_input_value: Sophis(1.00000000 + 87.23579967),
    //         output_count: 4,
    //         priority_fees: FeesExpected::receiver(Sophis(1.0)),
    //         // priority_fees: FeesExpected::None,
    //     })
    //     .finalize();
