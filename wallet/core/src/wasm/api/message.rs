 ...
 * Wallet API for interfacing with Rusty Sophis Wallet implementation.
 */
"#;
...
declare! {
     ...
     * with the default filename `sophis`.
     * 
     * @category Wallet API
     */
...
try_from! (args: IAccountsCreateRequest, AccountsCreateRequest, {
    ...
    let account_create_args = match kind.as_str() {
        ...
        _ => {
            return Err(Error::custom("only BIP32/sophis-keypair-standard accounts are currently supported"));
        }
    };
