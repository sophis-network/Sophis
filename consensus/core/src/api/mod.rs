...
pub trait ConsensusApi: Send + Sync {
    ...
    // -- Phase 6 — Data Availability accessors (sub-fase 6.4) --

    /// Reads one V5 carrier fragment by its 48-byte SHA3-384 payload_id.
    /// Returns `None` if the payload is unknown or pruned.
    fn da_get_payload(&self, _payload_id: [u8; 48]) -> Option<crate::da::PayloadEntry> {
        unimplemented!()
    }

    /// Returns the current bundle index (sorted payload_ids) for `bundle_id`.
    /// Reassembly of the actual byte payload is not done here — see service-layer.
    fn da_get_bundle_index(&self, _bundle_id: [u8; 48]) -> Option<crate::da::BundleIndex> {
        unimplemented!()
    }

    /// Returns the payload_ids of carriers accepted in `block_hash`.
    fn da_list_by_block(&self, _block_hash: Hash) -> Vec<[u8; 48]> {
        unimplemented!()
    }

    /// Returns the payload_ids in the (domain, blue_score_bucket) cell.
    fn da_list_by_domain(&self, _domain_byte: u8, _blue_score: u64) -> Vec<[u8; 48]> {
        unimplemented!()
    }

    /// Current chain blue score, used to compute confirmation counts.
    /// Defaults to 0 to keep existing test mocks compiling; the production
    /// consensus impl returns the sink blue score.
