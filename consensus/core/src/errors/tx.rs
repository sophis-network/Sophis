...
pub enum TxRuleError {
    ...
    #[error("transaction output {0} value is higher than the max allowed of {max}", max = MAX_SOMPI)]
    TxOutTooHigh(usize),
    ...
    #[error("token UTXO at output {0} is malformed: {1}")]
    TokenUtxoMalformed(usize, String),

    #[error("token conservation violated for token {0}: inputs={1} outputs={2}")]
    TokenConservationViolated(String, u64, u64),

    #[error("minting policy {0} is not deployed")]
    MintingPolicyNotDeployed(String),

    #[error("minting policy rejected the mint/burn: {0}")]
    MintingPolicyRejected(String),

    #[error("transfer policy {0} is not deployed")]
    TransferPolicyNotDeployed(String),

    #[error("transfer policy rejected the spend: {0}")]
    TransferPolicyRejected(String),

    #[error("V3 carrier output #{0} is malformed: {1}")]
    CarrierMalformed(usize, String),

    #[error("V3 carrier output #{0} must have value 0, got {1}")]
    CarrierNonZeroValue(usize, u64),

    #[error("transaction has {0} V3 carrier outputs where the max allowed per tx is {1}")]
    TooManyCarrierOutputs(usize, usize),
