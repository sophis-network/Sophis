...
pub struct Params {
    ...
    /// Orphan rate threshold for alert logging (0.0–1.0).
    /// The node logs a warning when the red-block rate over the last
    /// ORPHAN_RATE_WINDOW chain blocks exceeds this value.
    pub orphan_rate_alert_threshold: f64,

    /// Anti long-range attack floor — the minimum cumulative `blue_work` a header
    /// must carry before it can be promoted to `headers_selected_tip` (the
    /// "best known chain" pointer). Headers below this floor are still stored
    /// and indexed normally; only the chain-selection decision is gated.
    ///
    /// This value is **frozen at compile time** (never read from config or
    /// learned at runtime), so a freshly-bootstrapped node that has only seen
    /// a malicious low-work fork cannot be tricked into adopting it.
    ///
    /// Initial release ships every network with `BlueWorkType::ZERO`, which is
    /// the only value possible for a network that has not yet produced a
    /// single block at consensus time. Subsequent releases bump this value
    /// per network using a deterministic update procedure: read `blue_work`
    /// from the local node's selected tip at `now - 30 days` (well below
    /// finality depth so no honest reorg can affect it), commit the value to
    /// the constant, and ship a release. Operators who upgrade past the
    /// release window naturally inherit a stronger floor.
    pub min_chain_work: BlueWorkType,
}
...
impl Params {
    ...
    /// Based on the analysis at <https://github.com/sophisnet/docs/blob/main/Reference/prunality/Prunality.pdf>
    ...
    pub fn override_params(self, overrides: OverrideParams) -> Self {
        Self {
            ...
            orphan_rate_alert_threshold: self.orphan_rate_alert_threshold,
...
pub const MAINNET_PARAMS: Params = Params {
    // TODO(mainnet): add community-run DNS seeders before mainnet launch
    // Format: "seed1.sophis.network", "seed2.sophis.network", etc.
    // Each entry must point to a host running sophis-dnsseeder on port 46111.
    dns_seeders: &[],
    ...
    // Initial release: ZERO. Every release after the first stable mainnet uplift
    // bumps this to the local selected tip's `blue_work` from `now - 30 days`.
    // See doc on `Params::min_chain_work`.
    min_chain_work: BlueWorkType::ZERO,
};
...
pub const TESTNET_PARAMS: Params = Params {
    ...
    // Testnet ships ZERO since the same constant-update cadence applies. See
    // doc on `Params::min_chain_work`.
    min_chain_work: BlueWorkType::ZERO,
};
...
pub const SIMNET_PARAMS: Params = Params {
    ...
    // Simnet has no real chain to anchor against. See doc on
    // `Params::min_chain_work`.
    min_chain_work: BlueWorkType::ZERO,
};
...
pub const DEVNET_PARAMS: Params = Params {
    ...
    mass_per_script_pub_key_byte: 10,
    mass_per_sig_op: 1000,
    max_block_mass: 10_000_000, // increased for Dilithium-2 TXs (storage mass ~1M per TX)
    ...
    blockrate: {
        let mut p = BlockrateParams::new::<10>();
        p.coinbase_maturity = 20; // Devnet: 20 blocks (~7 min with RandomX) instead of 1000
        p
    },
    ...
    // Devnet is reset on every test run; floor stays ZERO. See doc on
    // `Params::min_chain_work`.
    min_chain_work: BlueWorkType::ZERO,
};
