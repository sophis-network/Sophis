use crate::{
    ...
    model::stores::{
        ...
        max_chain_work_seen::DbMaxChainWorkSeenStore,
...
use sophis_consensus_core::{BlockHashSet, blockstatus::BlockStatus};
...
pub struct ConsensusStorage {
    ...
    /// Anti long-range attack — persisted floor of the highest cumulative
    /// `blue_work` ever committed by this node's virtual state. The header
    /// processor refuses to promote a header to selected tip if its
    /// `blue_work` is below this floor. Updated atomically with virtual
    /// state commits.
    pub max_chain_work_seen_store: Arc<RwLock<DbMaxChainWorkSeenStore>>,
...
impl ConsensusStorage {
    pub fn new(db: Arc<DB>, config: Arc<Config>) -> Arc<Self> {
        ...
        // Phase 6 — DA store. Uses block_data cache budget (per-block fan-out
        ...
        // Anti long-range attack floor (persisted, monotone non-decreasing).
        let max_chain_work_seen_store = Arc::new(RwLock::new(DbMaxChainWorkSeenStore::new(db.clone())));
