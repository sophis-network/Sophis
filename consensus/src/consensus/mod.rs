...
use sophis_consensusmanager::{SessionLock, SessionReadGuard};
...
use sophis_svm_runtime::DbContractStore;
...
impl Consensus {
    pub fn new(
        ...
        let contract_store = Arc::new(DbContractStore::new(db.clone()));
...
impl ConsensusApi for Consensus {
    ...
    fn get_current_block_color(&self, hash: Hash) -> Option<bool> {
        ...
        while let Some(Reverse(SortableBlock { hash: decedent, .. })) = heap.pop() {
            if self.services.reachability_service.is_chain_ancestor_of(decedent, sink) {
                ...
                // unreachable.
                sophis_core::warn!("DAG topology inconsistency: {decedent} is expected to be a merging block of {hash}");
    ...
    fn get_chain_block_samples(&self) -> Vec<DaaScoreTimestamp> {
        ...
        if self.config.net.network_type == NetworkType::Mainnet {
            ...
            // Source: https://github.com/sophisgang/sophisd-py-explorer/blob/main/src/tx_timestamp_estimation.ipynb
            // For context see also: https://github.com/sophisgang/sophisd-py-explorer/blob/main/src/genesis_proof.ipynb
    ...
    fn calc_transaction_hash_merkle_root(&self, txs: &[Transaction]) -> MerkleHash {
    ...
    // -- Phase 6 — DA accessors (sub-fase 6.4) --

    fn da_get_payload(&self, payload_id: [u8; 48]) -> Option<sophis_consensus_core::da::PayloadEntry> {
        use crate::model::stores::da::DaStoreReader;
        use sophis_consensus_core::da::PayloadIdHash;
        self.storage.da_store.get_payload(PayloadIdHash(payload_id)).ok().flatten()
    }

    fn da_get_bundle_index(&self, bundle_id: [u8; 48]) -> Option<sophis_consensus_core::da::BundleIndex> {
        use crate::model::stores::da::DaStoreReader;
        use sophis_consensus_core::da::PayloadIdHash;
        self.storage.da_store.get_bundle(PayloadIdHash(bundle_id)).ok().flatten()
    }

    fn da_list_by_block(&self, block_hash: Hash) -> Vec<[u8; 48]> {
        use crate::model::stores::da::DaStoreReader;
        match self.storage.da_store.list_by_block(block_hash) {
            Ok(Some(bc)) => bc.payload_ids.into_iter().map(|p| p.0).collect(),
            _ => Vec::new(),
        }
    }

    fn da_list_by_domain(&self, domain_byte: u8, blue_score: u64) -> Vec<[u8; 48]> {
        use crate::model::stores::da::DaStoreReader;
        match self.storage.da_store.list_by_domain(domain_byte, blue_score) {
            Ok(Some(bucket)) => bucket.payload_ids.into_iter().map(|p| p.0).collect(),
            _ => Vec::new(),
        }
    }

    fn da_current_blue_score(&self) -> u64 {
        // Sink is the chain tip whose blue score equals the maximum confirmed
    ...
    fn get_logs(&self, filter: sophis_consensus_core::events::EventLogFilter) -> Vec<sophis_consensus_core::events::EventLog> {
        ...
        let pointers: Vec<sophis_consensus_core::events::EventLogPointer> =
            if let Some(idx) = filter.topics.iter().position(|t| t.is_some()) {
        ...
        for ptr in pointers {
            if Some(ptr.block_hash) != cached_block {
                cached_logs =
                    self.storage.event_store.get_logs_by_block(ptr.block_hash).ok().flatten().map(|l| l.logs).unwrap_or_default();
