...
use sophis_consensus_core::{api::ConsensusApi, config::Config, mining_rules::MiningRules};
use sophis_consensus_notify::root::ConsensusNotificationRoot;
use sophis_consensusmanager::{ConsensusFactory, ConsensusInstance, DynConsensusCtl, SessionLock};
use sophis_core::{debug, time::unix_now, warn};
use sophis_database::{
    prelude::{
...
impl ConsensusFactory for Factory {
    fn new_active_consensus(&self) -> (ConsensusInstance, DynConsensusCtl) {
        ...
        let db = sophis_database::prelude::ConnBuilder::default()
            .with_db_path(dir)
