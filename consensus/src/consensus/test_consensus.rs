...
use sophis_consensus_notify::{notification::Notification, root::ConsensusNotificationRoot};
use sophis_consensusmanager::{ConsensusFactory, ConsensusInstance, DynConsensusCtl};
use sophis_core::{core::Core, service::Service};
...
use crate::{
    ...
    model::{
        ...
        stores::{
            DB, ghostdag::DbGhostdagStore, headers::HeaderStoreReader, headers_selected_tip::DbHeadersSelectedTipStore,
            max_chain_work_seen::DbMaxChainWorkSeenStore, reachability::DbReachabilityStore, virtual_state::VirtualStores,
        },
    },
    params::Params,
...
impl TestConsensus {
    ...
    /// See `sophis_consensus_core::errors::block::RuleError` for the complete list of possible validation rules.
    ...
    /// Anti long-range attack — exposes the `headers_selected_tip` store so
    /// integration tests can assert promotion gating (Feature 1).
    pub fn headers_selected_tip_store(&self) -> &Arc<RwLock<DbHeadersSelectedTipStore>> {
    ...
    /// commits (Feature 2).
    pub fn max_chain_work_seen_store(&self) -> &Arc<RwLock<DbMaxChainWorkSeenStore>> {
        &self.consensus.storage.max_chain_work_seen_store
    }
}
