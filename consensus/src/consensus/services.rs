...
use crate::{
    ...
    processes::{
        ...
        sync::SyncManager,
        transaction_validator::{SvmContext, TransactionValidator},
...
impl ConsensusServices {
    pub fn new(
        ...
        let transaction_validator = {
            ...
            match SvmContext::new(contract_store) {
                // Phase 6 — wire the DA store so contracts that requested
                ...
                Ok(svm_ctx) => tv.with_svm(
                    svm_ctx
                        ...
                        .with_headers_store(storage.headers_store.clone())
                        .with_pruning_point_store(storage.pruning_point_store.clone()),
                ),
                Err(e) => {
