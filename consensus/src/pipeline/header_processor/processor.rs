use crate::{
    ...
    model::{
        ...
        stores::{
            ...
            max_chain_work_seen::{DbMaxChainWorkSeenStore, MaxChainWorkSeenStoreReader},
...
pub struct HeaderProcessor {
    ...
    /// Anti long-range attack — hardcoded floor; see `Params::min_chain_work`.
    pub(super) min_chain_work: BlueWorkType,
    ...
    /// Anti long-range attack — persisted floor that the chain-selection
    /// promotion check consults at every HST update attempt.
...
impl HeaderProcessor {
    pub fn new(
        ...
        Self {
            ...
            min_chain_work: params.min_chain_work,
        }
    }
    ...
    fn commit_header(&self, ctx: HeaderProcessingContext, header: &Header) {
        ...
        {
            // Anti long-range attack — gate chain-selection promotion on two
            // independent floors. The block itself is already committed
            // (ghostdag, headers, reachability); we only refuse to promote
            // it to selected tip until both floors are satisfied. This is
            // the single decision point where "this chain becomes the best
            // known" actually happens, and is the cheapest place to refuse
            // a low-work peer chain without poisoning local state.
            //
            // Floor #1: hardcoded `Params::min_chain_work` (per-network
            //           constant; ZERO on the initial release).
            // Floor #2: persisted `max_chain_work_seen` (monotone
            //           non-decreasing local floor — never accept a chain
            //           lower than what we have already validated).
            //
            // Both checks use direct `<` comparison. There is no margin
            // factor: `finality_depth` already absorbs natural reorg
            // ambiguity and adding one here would be a discretionary policy
            // dial we don't want.
            let max_seen = self.max_chain_work_seen_store.read().get();
            if header.blue_work < self.min_chain_work {
                warn!(
                    "Long-range protection — refusing to promote header {} to selected tip: blue_work={} < hardcoded min_chain_work={}",
                    ctx.hash, header.blue_work, self.min_chain_work
                );
            } else if header.blue_work < max_seen {
                warn!(
                    "Long-range protection — refusing to promote header {} to selected tip: blue_work={} < persisted max_chain_work_seen={}",
                    ctx.hash, header.blue_work, max_seen
                );
            } else {
