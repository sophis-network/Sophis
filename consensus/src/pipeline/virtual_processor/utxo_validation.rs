...
use crate::processes::transaction_validator::tx_validation_in_isolation::{is_contract_deploy, is_mint_policy_deploy};
use crate::{
    errors::{
        BlockProcessResult,
        RuleError::{
...
use sophis_core::{info, trace};
use sophis_hashes::{Hash, MerkleHash};
use sophis_muhash::MuHash;
use sophis_svm_core::{ContractDeployPayload, MintingPolicyPayload, hash_mint_policy, hash_wasm};
use sophis_utils::refs::Refs;
...
pub(crate) mod crescendo {
    use sophis_core::{info, log::CRESCENDO_KEYWORD};
    use std::sync::{
        Arc,
...
impl VirtualStateProcessor {
    ...
    pub(super) fn calculate_utxo_state<V: UtxoView + Sync>(
            ...
            .enumerate()
        {
            ...
            for (validated_tx, _) in validated_transactions.iter() {
                ...
                // Store WASM bytecode when a contract or minting policy deploy tx is accepted
                self.process_contract_deploy_if_needed(validated_tx.tx);
                self.process_mint_policy_deploy_if_needed(validated_tx.tx);
            }
    ...
    /// If `tx` is a contract deploy, extract WASM from payload and store it.
    /// Called once per accepted tx — silent no-op for normal transactions.
    fn process_contract_deploy_if_needed(&self, tx: &Transaction) {
        if !is_contract_deploy(tx) {
            return;
        }
        let svm = match self.transaction_validator.svm.as_ref() {
            Some(s) => s,
            None => return,
        };
        let payload = match ContractDeployPayload::try_from_slice(&tx.payload) {
            Ok(p) => p,
            Err(e) => {
                // Should not happen — isolation validation already verified the payload
                info!("contract deploy payload decode failed for tx {}: {e}", tx.id());
                return;
            }
        };
        let contract_id = hash_wasm(&payload.wasm);
        svm.store.as_ref().deploy_if_absent(contract_id, payload.wasm);
        info!("deployed contract {contract_id:?} from tx {}", tx.id());
    }

    /// If `tx` is a Minting Policy deploy, stores the WASM in the contract store.
    /// Idempotent — re-orgs may replay deploy txs safely.
    fn process_mint_policy_deploy_if_needed(&self, tx: &Transaction) {
        ...
        let payload = match MintingPolicyPayload::try_from_slice(&tx.payload) {
            Ok(p) => p,
            Err(e) => {
                info!("mint policy deploy payload decode failed for tx {}: {e}", tx.id());
                return;
            }
        };
        let token_id = hash_mint_policy(&payload.wasm);
        svm.store.as_ref().deploy_if_absent(token_id, payload.wasm);
        info!("deployed minting policy for token {token_id:?} from tx {}", tx.id());
    }
    ...
    pub(super) fn calc_accepted_id_merkle_root(
        ...
        sophis_merkle::merkle_hash_from_node(
            self.headers_store.get_header(selected_parent).unwrap().accepted_id_merkle_root,
            sophis_merkle::calc_merkle_root(accepted_tx_ids),
        )
    }
}
