use crate::{
    ...
    model::{services::reachability::ReachabilityService, stores::headers_selected_tip::HeadersSelectedTipStoreReader},
};
...
use crate::model::stores::max_chain_work_seen::MaxChainWorkSeenStoreReader;
...
// ---- Anti long-range attack — integration tests ---------------------------
//
// These two tests exercise the `min_chain_work` (Feature 1) and
// `max_chain_work_seen` (Feature 2) gating wired in `commit_header` and
// `commit_virtual_state`. They share the same chain construction path used
// by the rest of this module, so PoW is mocked away and the sole signal is
// `blue_work` arithmetic.

#[tokio::test]
async fn max_chain_work_seen_advances_with_chain() {
    // Validates Feature 2 (`max_chain_work_seen`) — the persisted floor must
    // monotonically advance to track each new virtual-state commit. This is
    // the consensus-side guarantee that a fresh peer cannot roll us back to
    // an earlier-work chain after we have already validated a higher-work
    // one locally.
    let config = ConfigBuilder::new(MAINNET_PARAMS).skip_proof_of_work().build();
    let mut ctx = TestContext::new(TestConsensus::new(&config));

    // After consensus init, genesis has been processed and virtual state is
    // already committed once. The floor reflects genesis's blue_work; it is
    // not strictly ZERO. We anchor against this snapshot and assert the
    // floor only ever advances from here.
    let initial_floor = ctx.consensus.max_chain_work_seen_store().read().get();

    // Mine a chain. Each block bumps the virtual state's `blue_work`; the
    // commit_virtual_state hook must fold that into the floor inside the
    // same WriteBatch.
    for _ in 0..10 {
        ctx.build_block_template_row(0..1).validate_and_insert_row().await.assert_valid_utxo_tip();
    }

    let after_floor = ctx.consensus.max_chain_work_seen_store().read().get();
    assert!(after_floor > initial_floor, "floor must advance after a non-trivial chain ({initial_floor} -> {after_floor})");

    // Every additional block can only raise (or keep) the floor — never
    // lower. Sample a few more rounds to catch any regression.
    let mut prev_floor = after_floor;
    for _ in 0..5 {
        ctx.build_block_template_row(0..1).validate_and_insert_row().await.assert_valid_utxo_tip();
        let new_floor = ctx.consensus.max_chain_work_seen_store().read().get();
        assert!(new_floor >= prev_floor, "floor regressed: {prev_floor} -> {new_floor}");
        prev_floor = new_floor;
    }
}

#[tokio::test]
async fn min_chain_work_blocks_hst_promotion() {
    // Validates Feature 1 (`min_chain_work`) — when the hardcoded floor is
    // higher than what any block in the chain can carry, no header can win
    // promotion to `headers_selected_tip`. The HST must remain pinned at
    // genesis even though normal block validation succeeds and the rest of
    // the pipeline progresses.
    //
    // We pick a min_chain_work that is enormous compared to anything a
    // mocked-PoW chain produces (`Uint192::MAX`) — this guarantees the
    // hardcoded floor wins regardless of difficulty calibration drift.
    let config = ConfigBuilder::new(MAINNET_PARAMS)
        .skip_proof_of_work()
        .edit_consensus_params(|p| {
            p.min_chain_work = BlueWorkType::MAX;
        })
        .build();
    let mut ctx = TestContext::new(TestConsensus::new(&config));

    let genesis_hash = ctx.consensus.params().genesis.hash;
    let initial_hst = ctx.consensus.headers_selected_tip_store().read().get().unwrap();
    assert_eq!(initial_hst.hash, genesis_hash, "before mining, HST is genesis");

    // Mine a non-trivial chain. Headers are validated and stored; only the
    // promotion to selected tip is gated.
    for _ in 0..10 {
        ctx.build_block_template_row(0..1).validate_and_insert_row().await;
    }

    let after_hst = ctx.consensus.headers_selected_tip_store().read().get().unwrap();
    assert_eq!(
        after_hst.hash, genesis_hash,
        "with min_chain_work = MAX no header can be promoted to selected tip; HST must stay at genesis (got {})",
        after_hst.hash
    );
}
