...
use sophis_consensus_core::BlockHashSet;
use sophis_consensus_core::{BlockHashMap, BlockHasher, BlockLevel, blockhash::BlockHashes};
use sophis_database::prelude::{BatchDbWriter, CachePolicy, DbWriter};
use sophis_database::prelude::{CachedDbAccess, DbKey, DirectDbWriter};
use sophis_database::prelude::{DB, StoreResult};
use sophis_database::prelude::{DirectWriter, MemoryWriter};
use sophis_database::prelude::{ReadLock, StoreError};
use sophis_database::registry::{DatabaseStorePrefixes, SEPARATOR};
...
impl RelationsStoreReader for StagingRelationsStore<'_> {
    ...
    fn counts(&self) -> Result<(usize, usize), StoreError> {
        let count = self
            ...
            .iterator()
            .map(|r| r.unwrap().0)
            .map(|k| <[u8; sophis_hashes::HASH_SIZE]>::try_from(&k[..]).unwrap())
            .map(Hash::from_bytes)
...
mod tests {
    ...
    use sophis_utils::mem_size::MemMode;
    ...
    fn test_db_relations_store() {
        let (lt, db) = create_temp_db!(sophis_database::prelude::ConnBuilder::default().with_files_limit(10));
