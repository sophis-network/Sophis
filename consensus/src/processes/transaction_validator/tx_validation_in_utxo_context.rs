...
impl TransactionValidator {
    ...
    fn check_alt_references(&self, tx: &impl VerifiableTransaction) -> TxResult<()> {
        ...
        for (i, output) in tx.outputs().iter().enumerate() {
            ...
            //
            // F-27-A — pruning stability. `selected_chain_store` only retains
            // entries above the pruning point; blocks finalized below it are
            // stripped from that store even though they remain permanently
            // canonical. When `get_by_hash` returns KeyNotFound we fall back to
            // a DAA-score comparison against the current pruning-point DAA score:
            //   creating_daa_score < pp_daa_score
            //     → below the finality horizon → permanently canonical → accept.
            // This is safe because (a) ALT indexing only fires during canonical-
            // chain commits so the creating block was definitely canonical at
            // index time, and (b) blocks below the pruning point cannot be
