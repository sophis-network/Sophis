...
use sophis_consensus_core::header as native;
...
impl Header {
    ...
    pub fn set_hash_merkle_root_from_js_value(&mut self, js_value: JsValue) {
        self.inner_mut().hash_merkle_root = MerkleHash::from_slice(&js_value.try_as_vec_u8().expect("hash merkle root"));
    }
    ...
    pub fn set_accepted_id_merkle_root_from_js_value(&mut self, js_value: JsValue) {
        self.inner_mut().accepted_id_merkle_root = MerkleHash::from_slice(&js_value.try_as_vec_u8().expect("accepted id merkle root"));
    }
        ...
        Self::resolve(value, || {
            if let Some(object) = Object::try_from(value.as_ref()) {
                ...
                let header = native::Header {
                    ...
                    parents_by_level,
                    hash_merkle_root: {
                        let s: String = object.get_string("hashMerkleRoot")?;
                        MerkleHash::from_str(&s).map_err(|e| Error::convert("hashMerkleRoot", e))?
                    },
                    accepted_id_merkle_root: {
                        let s: String = object.get_string("acceptedIdMerkleRoot")?;
                        MerkleHash::from_str(&s).map_err(|e| Error::convert("acceptedIdMerkleRoot", e))?
                    },
