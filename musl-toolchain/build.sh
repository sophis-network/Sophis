...
if [ -d "$HOME/x-tools" ] && [ -f "$PRESET_HASH_FILE" ] && [ "$(cat $PRESET_HASH_FILE)" = "$CURRENT_PRESET_HASH" ]; then
  ...
  # For private repos, use the API endpoint with $GITHUB_TOKEN auth (the public
  # /releases/download/... URL returns 404 for assets in private repos).
  try_download_toolchain() {
    local repo="$1"
    echo "Trying to download toolchain from $repo..."

    if [ -n "${GITHUB_TOKEN:-}" ]; then
      # Auth path — works for both public and private repos.
      local asset_id
      asset_id=$(curl -fsSL \
        -H "Authorization: Bearer $GITHUB_TOKEN" \
        -H "Accept: application/vnd.github+json" \
        "https://api.github.com/repos/$repo/releases/tags/$TOOLCHAIN_TAG" \
        2>/dev/null | grep -oE '"id":[[:space:]]*[0-9]+' | head -1 | grep -oE '[0-9]+')
      if [ -z "$asset_id" ]; then
        echo "  No release found in $repo"
        return 1
      fi
      # The first .id is the release id; we want the first asset id, which is on a later line.
      asset_id=$(curl -fsSL \
        -H "Authorization: Bearer $GITHUB_TOKEN" \
        -H "Accept: application/vnd.github+json" \
        "https://api.github.com/repos/$repo/releases/tags/$TOOLCHAIN_TAG" \
        2>/dev/null | python3 -c 'import sys, json; data = json.load(sys.stdin); a = next((x for x in data.get("assets", []) if x.get("name") == "x-tools.tar.zst"), None); print(a["id"] if a else "")')
      if [ -z "$asset_id" ]; then
        echo "  No x-tools.tar.zst asset in $repo release"
        return 1
      fi
      if ! curl -fsSL \
        -H "Authorization: Bearer $GITHUB_TOKEN" \
        -H "Accept: application/octet-stream" \
        -o /tmp/x-tools.tar.zst \
        "https://api.github.com/repos/$repo/releases/assets/$asset_id"; then
        echo "  Asset download failed for $repo"
        rm -f /tmp/x-tools.tar.zst
        return 1
      fi
    else
      # Unauth path — works only for public repos.
      ...
      if ! curl -fsSL -o /tmp/x-tools.tar.zst "$download_url"; then
        echo "  No release found in $repo (unauth, repo may be private)"
        rm -f /tmp/x-tools.tar.zst
        return 1
      fi
    fi
